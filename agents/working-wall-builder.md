...
description: Working-wall builder. Renders a `working-wall.json` (produced by working-wall-designer) into a single .pdf by running the fixed build script. Mechanical execution only - no pedagogy, no card design, no reworded content. Use after working-wall-designer has produced working-wall.json with at least one card.
model: haiku
color: "#3CB371"
---
