...
2. Read the output for these signals:
   ...
   | `LAYOUT_PREFLIGHT_FAILED` / `CONTENT_ZONE_INCOMPATIBLE` / `STEP_TEXT_OVERLOAD` | Found by the layout preflight, which draws every slide once before anything is written. Names the slide. Nothing is published while one stands. |
