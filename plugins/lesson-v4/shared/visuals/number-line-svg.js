...
function normalise(data = {}) {
  ...
  return lines.map((s) => {
    ...
    return { start, end, intervals, showTicks: s.showTicks !== false, arrows, tickLabels, questionState };
  });
}
function describeLayout(data = {}) {
  ...
  const rows = specs.map((s, r) => {
    ...
    const arrowBand = arrows.length ? ARROW_GAP + ARROW_STEM + ARROW_HEAD + tiers.length*FONT_MAX*LINE_HEIGHT : WRITE_HEIGHT;
    ...
    const row = { ...s, x1, x2, y, top, bottom, arrows, labels,
