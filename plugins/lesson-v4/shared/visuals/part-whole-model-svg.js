...
// The real coins, placed by the shared money picture, so a coin in a part-whole
// model is the coin the class sees in a money strip (13 September 2026).
const money = require('./money-svg');

function coinImage(denomination, x, y, w, h) {
  return money.coinImage(denomination, x, y, w, h);
}

function denominationMm(denomination) {
  const coins = money.COIN_MM;
  const notes = money.NOTE_MM;
