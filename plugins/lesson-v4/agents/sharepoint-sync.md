...
Return those results in plain English. A `SKIPPED=` line is expected, not a fault: the teacher's drive takes only the teaching resources (deck, worksheets, answer key, working wall, stick-in sheets), so a run report or walk-through stays in the output folder. If the E: drive is unavailable or any
