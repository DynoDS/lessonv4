...
Read `[PLUGIN_ROOT]/references/preferences.md` too, at the start of every run - a stick-in piece is a surface a child reads and writes on, so the house rules for child-facing work reach it like any other. Read the introduction and contents page, then your sections: Written Voice and Question Labelling. The rest of the file governs the lesson upstream of the pack; when unsure whether a section touches a piece you are emitting, read it too.
...
2. **Apply the write-on test to each moment**: *does the child write onto a figure they could not redraw by hand?* A Venn to sort into, a grid to plot on, a diagram to label passes; an answer the child records in their own hand - an arithmetic result, a fluency list, a reasoning sentence - never does. Holding the pack to the moments that genuinely need it is what keeps it worth the paper, and a lesson with none is a correct and common outcome.
