...
- success criteria are usable actions, decisions or recognition categories that match the taught performance, rather than a list of facts or a lesson outline. Read the object, not its id: a `reference-table` whose rows are facts the task uses (nutrients and their jobs) is a representation wearing the criteria label, and the real criteria are then missing; a `steps` list carrying a condition as a step has a branch that belongs in a lookup beside it. Both are returned, because the slide designer must render what it is given;
