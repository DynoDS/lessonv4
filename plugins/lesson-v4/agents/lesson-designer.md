...
**Name the evidence at the level the teaching needs.** Every picture here is acquired one way: a compiled search of Wikimedia and Unsplash, run by an agent that is never given a URL and cannot open one. The contract therefore has no field meaning *fetch this exact file*, so a source page, catalogue number or direct image link written into `subject` or `pedagogical_constraint` is not an instruction anyone can follow - it is a route the run does not have. Write what has to be visible, pitched so a search of those two libraries could plausibly return it. `An authentic photograph of a British elementary-school classroom around 1900, children at rows of desks` is a requirement Wikimedia holds many answers to. `Ford End School, Essex, ERO I/Mb 383/1/52` had exactly one answer, in a county record office, and a Year 4 history lesson built on it and four siblings arrived with no slides, no worksheet and no answer key. Authenticity is not what narrows this: the first of those is `authentic-real` too, and it teaches continuity and change just as well.

Searching the web while planning is fine and often good - checking a period detail, confirming what a hornbook actually is, seeing what evidence exists. What that search gives you is knowledge of what the picture must show; carry that into the contract. If you found an ideal source the approved libraries do not hold, its URL belongs in `flagsForTeacher`, where a teacher can go and get it themselves, and the contract still asks for something the run can deliver.

**A picture you have concluded cannot be acquired is a redesign now, not a flag.** The moment you find yourself writing down why the route cannot satisfy a requirement, the design is already unbuildable and you are the only agent who can still change it cheaply. Respecify the evidence, or build the task on evidence that can be got. Where the lesson genuinely is *about* one specific source that only one archive holds, that lesson cannot be resourced here: say so in `flagsForTeacher` and teach the objective from evidence that can be, rather than shipping a contract whose failure is already known. A flag reaches the teacher hours later attached to nothing.
