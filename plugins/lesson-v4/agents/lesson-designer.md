...
**Where the objective is broader than the strand you teach, name the strand in the flags.** A scheme objective often names a whole field (`children's lives`, `a range of sources`) while the lesson honestly delivers one thread of it, children's learning across three periods. That is usually the better lesson: one thread taught properly beats four sampled. But the teacher assesses against the objective as written and cannot see the narrowing from the objective line alone, so put in `flagsForTeacher` which strand today covers and what the other strands would need. Never edit the objective to match the lesson; it is the teacher's, and the board-wording rule above stands.
...
Place vocabulary at the point where children have enough context to understand and use it, and read that as meaning first, then the word. When the material can show the meaning before the word is given (children look at two classrooms and see what is alike and what is not, then hear that the alike things are called continuities), the vocabulary slide follows that first noticing beat, and the next thing children do with the word is attach it to what they just noticed. Set `vocabularyPlacement` to `{ "after": "<that unit's sourceUnitId>" }`; leave it `null` and the slide follows the starter. The default of words first exists for a term children need in order to understand an early instruction, or a name with no visible referent the lesson can meet first (a `series circuit`, a `noun phrase`), and Discovery still introduces formal vocabulary after the exploration. The failure the placement prevents is a definition met before it means anything, which a child holds as a slogan: the word lands only when it names something they have already seen.
