...
**Choose response form and demand separately.** Every Do beat: every child uses the chunk and leaves something the teacher can see (written answer, partner's spoken response, visible decision, physical position). Match form to what was taught: fact = recall or sort; process = label, sketch, sequence; concept = write or generate; value = rank or talk. Vary form when improves learning/attention/access, not quota. Demand climbs across lesson where the objective supports it; `preferences.md` owns that judgement across teaching responses and main practice together. A short recall response may secure new knowledge, including in the last Do. Read what children actually do; do not label recall as reasoning or force each response to be harder.
...
**Criteria say what the child does or what good work shows; a table of facts the task uses is a representation.** The nutrient table children read while planning a lunch is `rep-00x`; the criteria are what the plan must do. Recognition (name this tooth, this turn) is the one case where the labelled set is the criteria. Keep the panel a glance: five steps at most, cells of label length; a condition inside a step is a branch and goes in a reference-table criteria referenced beside the steps. `preferences.md` → Success Criteria carries the cases.
