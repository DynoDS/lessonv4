...
- for each taught idea, the child-facing lines that carry its meaning, reason and example on the board rather than only in the script; and for a substantial task, how it is launched (what the class has established, the example and non-example of the product, the steps) or why its question alone is enough;
