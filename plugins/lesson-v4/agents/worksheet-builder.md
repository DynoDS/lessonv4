...
2. Read the output for these signals:
   ...
   | `AUTO_LAYOUT: ...` | Which page shape the engine chose for a sheet whose designer left the choice to it, and how full it runs. Information, not a fault - pass it through so the record shows which shape was chosen. |
   ...
   | `IMAGE_MISSING` | A photograph the spec names could not be read. The build prints one of these for **every** unreadable picture, so report every one it printed: the run gets a single repair round, and a sheet missing three photographs that hears about them one build at a time never gets past the first. For each filename, say **whether it already has a terminal picture receipt**, because the two cases go to different owners and only one of them can be fixed. No terminal receipt, or one that published: the file should be there, so a focused one-filename picture repair runs for that photo, not the designer. A terminal receipt reading `unsatisfied` or `omitted`: sourcing for it is closed and no repair can produce it, so the picture route is a dead end and the worksheet-designer re-authors that one reference instead. Reporting only the filename sends the run down the dead end and the sheets are lost. |
