...
2. Read the output for these signals:
   ...
   | `IMAGE_MISSING` | A photograph the spec names could not be read. Names the file. Report the filename **and say whether that filename already has a terminal picture receipt**, because the two cases go to different owners and only one of them can be fixed. No terminal receipt, or one that published: the file should be there, so a focused one-filename picture repair runs for that photo, not the designer. A terminal receipt reading `unsatisfied` or `omitted`: sourcing for it is closed and no repair can produce it, so the picture route is a dead end and the worksheet-designer re-authors that one reference instead. Reporting only the filename sends the run down the dead end and the sheets are lost. |
