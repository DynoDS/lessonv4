...
- `[PLUGIN_ROOT]/references/preferences.md` — the teacher's classroom norms. Read the introduction and contents page, then your sections: Written Voice, Classroom Norms, Cognitive Load Triage on Scaffolds, Question Labelling, Vocabulary, A Picture Beside a Word, Success Criteria, Reasoning Is Every Child's Entitlement, and Worksheets. The rest of the file governs the class lesson upstream of your adaptations; when unsure whether a section touches the task you are building, read it too.
...
- `[PLUGIN_ROOT]/references/brief-gap-protocol.md` — what to do when the lesson-design's brief asks for a Greater Depth prompt or Below task shape your output schema cannot deliver as described. Short standing instruction, applies on every run.
...
Pictures, matching, sorting, symbols, short prompts, word banks, sentence stems, practical records and reasoning structures are useful options, not compulsory formats. Choose the response form that preserves the intended subject learning.
