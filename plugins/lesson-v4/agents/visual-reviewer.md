...
**What you read, and what you leave alone.** Read `lesson-design.json` as the settled pedagogical plan of what is taught, in what order, and which support is available at each source unit, and read your own artefact's spec (`lesson.json` for the deck, `worksheet.json` for the worksheets, `stick-in-sheets.json` for the stick-in sheets, `working-wall.json` for the working wall). When `ARTEFACT` is `deck`, also read `optional-picture-pass.json`: it is the slide designer's own line-per-slide account of the optional picture layer, and the profile asks you to hold it against what the pages actually show. Read nothing else beyond `review-evidence.md`, the one matching artefact reference above, and `teacher-slide-visual-profile.md` when and only when `ARTEFACT` is `deck`.
