...
2. Read the output for these signals:
   ...
   | `LAYOUT_PREFLIGHT_FAILED` / `CONTENT_ZONE_INCOMPATIBLE` / `STEP_TEXT_OVERLOAD` / `SC_PANEL_TOO_LARGE` | Found by the layout preflight, which draws every slide once before anything is written. Names the slide. That slide is left blank and the rest of the deck is still built and checked, so any other fault (text that does not fit, a missing picture) is listed in the same output; report them all together. Nothing is published while one stands. |
