...
## 2026-09-22 - The example is written when the board does not make it obvious (4.2.278)

*From Daniel's read of the rebuilt Week 4 science deck, hand-written to the new four-part Teach shape.*

- Three of its four Teach boards carried the example as an instruction to look (`Follow the tube down from the mouth on the diagram`, `Find the very end of the large intestine on the diagram`, `Follow the oesophagus down`), on a diagram that already labelled the very parts children were being sent to find. Daniel's rule, in his words: the example is fine spoken when it is obvious, written when it is not, and probably written when there is no picture to point at at all.
- `teaching-sequence-content-based.md` → `explanation` part 3 now carries it with its reason and both limits: obvious means the only thing on the board or a thing the picture's own label names, and what is written is what the class will notice rather than the instruction to go and look. The Tudor calibration (`Look at her. She isn't being paid to do this.`) is protected by name, because the noticing follows the look there.
...
## 2026-09-22 - The guidance arrives, the rules that pushed the other way say the opposite, and three faults are checked (4.2.283)

*From the Year 4 History Week 5 deck `How did children's leisure time change?`, built in Codex on 4.2.276 and given to a newly qualified teacher on her first day. She could not tell what to do with it. The teacher's own read, slide by slide, named five faults: the board assumes children know Elizabeth I, a government, an order, a `modern summary`, the Thames and a steam engine, and asks adult questions; no slide says why it is there; the Do slides are "pick the sentence I just said"; the written explanation arrives with no model and no practice; and it is pitched at Year 9 (what a government order does and does not prove). "It's basically like a robot just said some stuff to a human, thinking it all makes sense."*

- **What the investigation found, from the run's own Codex records.** Most of the guidance for these faults was already written and never arrived, or arrived and was cancelled. Codex caps each command's output at about 10,000 tokens and removes the MIDDLE, marking the gap, while the head and the success line at the end still arrive. Every lesson designer pass read `lesson-designer.md` whole and lost 26,616 of its 36,408 tokens; `preferences.md`'s `Read the final task backwards` and `Work from what children can use` reached no designer pass at all; the reviewer's always-read batch and the whole history file were cut the same way. Six role files are longer than one page (the designer, the reviewer, and the slide, worksheet, adaptation and wall designers), so every one of those workers has been losing its middle on Codex. The friction line "used smaller section reads to recover the required guidance - run unharmed" was not true, and neither was this morning's plan note that each truncated read "recovered by reading in smaller pieces".
- **Where the rules pushed the other way.** History's doing-versus-thinking test named "what it can and cannot tell us" as the thought, in the designer and the reviewer; the history sketch called its source caution "the idea the lesson is built to land"; the Do catalogue's `Which claim does this support?` produced `every` and `nothing` strawmen and was used by name in both Week 4 and Week 5 history; "label sources honestly" put `modern summary` and organisation names on child boards; a quick check that restated the Teach was excused in seven places by "name it as a check", and the reviewer used that excuse on the steam option bank it had noticed was passable; `preferences.md` said to judge a slide "not as a page that must teach an absent teacher's whole explanation" (6 September, a modelling rule read as a slide rule, beside the 14 September day-one-teacher standard); and the reviewer's notes-closed read had one repair, move it onto the board, which is how the Tudor order grew a slide of its own.
- **Two coexisting faults in the run itself, recorded as evidence.** The leisure lesson was designed inside a redesign pass over a wrong-row Maya design, reusing its seven-unit skeleton, and its final task was swapped late (peepshow to charades) after picture losses with no one re-reading the lesson whole. And the run report's timeline and launch audit read the Maths 17 orchestrator, which was building at the same time, so the "designer 2 minutes" recorded for this lesson was another lesson's.

The changes:

- **Pages (mechanism).** `read-reference.py` never prints more than 24,000 characters in one call: a longer read prints page 1 of N, and only the last page prints `REFERENCE_READ_OK`; the others print `REFERENCE_READ_PARTIAL` and the command for the next page. `--role NAME` reads a role file and `--file PATH` any Markdown file (the review view, a subject file) through the same pager. Each of the six long role files now opens, under its title where Codex never cuts, with the instruction to read itself in pages. The designer's subject and route reads, and the reviewer's card commands, subject file, route file and view, are paged. Tests: pages rejoin exactly, stay under the limit, break between paragraphs, and every live role file pages cleanly.
- **Turned round (guidance, at each owner).** History thinking is what the source shows about the people and why (designer, reviewer); the sketch's caution serves the lesson's question and never becomes it; a source beat's thinking names what it shows about the people; `Say what the source cannot tell us` is for a lesson whose objective is sources; a source that costs more explaining than it teaches is replaced, with an operational test (list what a new teacher must explain against what the next step uses); honest labels in words the class has, provenance in `teacherInfo`; a named person or place is explained in a clause where it first appears. A plan's named source is activity, not coverage (designer). `A quick check is a fresh case, not the last slide again` now owns the boundary in `preferences.md`, in the teacher's 14 September ruling, and the excuse is gone from the designer, the reviewer, `preferences.md` and the catalogue; `Which claim` is for evidence children read for themselves, with believable wrong options. The absent-teacher sentence is scoped to modelling slides; task framing in the notes is not a Teach slide's because. The reviewer's notes-closed repair has two directions: on the route, onto the board; a detour, out of the lesson.
- **A clash from this morning resolved.** 4.2.277 made the once-rule refuse a headline repeated through `stickyKnowledgeRefs`, while the content route and the output template (4.2.223) still told the designer to record it there. Both now say the unit whose headline is the fact leaves it out of its own `stickyKnowledgeRefs`; later beats reference it.
- **Checked (code).** `validate-lesson-design.py` refuses a content, discovery, dialogic or task-centred Practise that asks for a written explanation or comparison when its launch shows no good instance and no earlier beat has shown the class one (a modelled turn, a revealed model answer or a launch); a skill lesson is exempt because its turns model the move. Run over the 54 saved designs it refuses this lesson, three older ones with the same gap (Tudor children, a Week 1 continuity run, the being-active PSHE lesson) and one unfinished scaffold, and passes every lesson that launched with a model. `check-slide-design.js` refuses `SAME_LINE_TWICE_ON_ONE_SLIDE` (the steam slide, where a repair added the star line to satisfy the landed-sentence check and left the card); parallel cards and per-figure notes are not repeats, and across 47 decks only the steam slide trips it. `worker-launch.py audit|timeline --working-dir` reads the run's own record, and without it a concurrent run is named in a `WORKER_RUN_NOTE` line rather than reported as this run's; `SKILL.md` passes the folder. The playbook's own copy of the audit command is unchanged because it has nine bytes left.
- **The reviewer's page.** The review view gains `Names on the board`, every name the class reads with where it first appears (Elizabeth I, Thames, English Heritage and NSPCC here; Lord Ashley, Ibbotson & Co, the Ragged School Union and the Shaftesbury Society in Week 4), and `Each Do beside the teaching before it`, the expected answer next to the Teach it follows with how many of its words the Teach already said (8 of 8 for the steam option bank). The reviewer accounts for each name and reads each Do there. A word-overlap refusal was measured over the saved decks and rejected: it flagged good Dos that rightly use the taught words and missed the order Do's paraphrase.

**Not done, and why.** The effort setting (Codex designer and reviewer at `low` since 4.2.239) is the teacher's choice and was left; the history of fixes shows the same faults at higher settings, so it is an aggravator, not the root. The consolidation of `lesson-designer.md` and `preferences.md` is still owed and still blocking playbook changes. Nothing forces the walk-through to be written before the contract, or a redesign to re-read the whole lesson.

**Evidence.** Python 2067 pass; builder 709, worksheets 716, shared 172, stick-in 70, wall 142, voice harness 21. Deterministic checks on this run's own files: the validator refuses its design on the explanation task and the repeated headline; the slide check refuses slide 8; the view lists its names and shows both Do restatements. **Behaviour is not yet validated**: the acceptance test is a fresh, untouched Codex run of this same lesson on this version, read by the teacher against the five faults.

## 2026-09-22 - Vocabulary is written once, in the teacher's words, and every rule is pinned (4.2.284)

*The trial topic of the streamline in `plans/2026-09-22-streamline-brief.md`: vocabulary, taken through every step before the method touches the big files.*

- **The inventory.** Every place the plugin tells an agent anything about vocabulary was listed with its own words, when it applies, its strength, the names code depends on, and its kind: 285 places in 40 files (`plans/2026-09-22-vocabulary-ledger.md`). A script proves every quote exists at its file and line. An independent agent that had not written the list then searched for what it missed and found 28 rules, 20 "copies" that carried an extra condition, and five disagreements; each was checked against the files and added. The first pass had missed about one rule in ten, which is the reason the method has a second reader.
- **What the list showed.** The Lesson Designer's own section and `preferences.md` → Vocabulary each carried most of the core rules, in different words. Nine places pulled against each other in what the designer reads; five more are in the wall and slide files and wait for those topics. Five pieces of text described how the plugin used to work. The answer-arrow preference was filed inside Vocabulary. The rule that a word needing real teaching gets its own Teach beat lived only in the slide designer's playbook, which the designer never reads.
- **The teacher's decisions**, all eighteen and two more in one message, are recorded in his words in the ledger. The ones that change what a lesson gets: maths cards use the lesson's own drawn picture, not an emoji that hints at the topic; a definition is "a sentence I'd say to the class ... conversational so children understand it", and the contract's and catalogue's own examples now are; big history words get a card just before they are needed and are then used; up to five words, never a floor of three; a card's word reaches the next beat's board, not only its script; a word big enough for its own Teach beat keeps its card too; in a skill lesson a method's words come before the My Turn, never inside the cycle; a set taught one member at a time brings each word in just before its own part; the wall keeps the lesson's own definition.
- **One home.** `preferences.md` → Vocabulary now holds each vocabulary rule once, with every extra condition the second copies carried (the objective's own term, pairs never made to fit the limit, one picture per word, reusing a photograph the teaching already sourced, the repair that fixes the earlier beat's wording, Discovery's formal vocabulary after the exploration, a prerequisite word and a contrast pair as two introductions). The designer's own section keeps how to record it and the instruction to read that section whole before choosing the set and again before writing `vocabularyIntroductions`. Stories became reasons, and a case that makes a rule clear stays only as a plain example, because, in the teacher's words, "if something has to tell it what happened in a specific lesson, then the rule's not really fixed at all". Out-of-date text went: the prose "visual note", "2 to 5 words" on a vocabulary slide, "the shape a definition stays inside" (nothing checks one), and the vocabulary-card gate removed on 13 September.
- **A story kept here as it left the runtime.** A Year 4 science lesson (18 September 2026) introduced decay, plaque and acid together after the starter; plaque and acid were needed in the next beat and decay not for another three, so the class met a definition, did two beats of other work, and met the thing it named later. It is the reason for the next-beat check shipped in 4.2.234, which had no entry here.
- **Checked (code).** `validate-lesson-design.py` now refuses a card whose word the next beat says only in its script: the word has to reach what that beat shows the class, which is its own content, the success criteria and sticky facts it puts on the slide, and the words its picture prints. Those printed words are read out of the picture's required features the ways designers write them (`Visible caption: Each interval is worth 10.`, `labels reading 'ear canal' and 'eardrum'`, `the eardrum labelled`, `columns labelled Thousands, Hundreds, Tens and Ones`, `a blank circle captioned thousands`, `halfway written under the middle tick`), while a feature that only describes the picture prints no word (`every integer labelled, with correct negative signs` shows minus signs, not the word `negative`; `No caption: ...` prints nothing). The matcher hears a word's plain forms (`continuities`, `valleys`, `changed`, `rounding`) without mistaking `roundabout` for `round`, `ruler` for `rule` or `counters` for `count`, and the older never-used and first-needed checks now read the same slide, so their message names the right repair. Across the 65 saved designs in the repository that plan introductions, 34 of 170 word introductions were script-only on the next beat: continuity and change on history Teach beats, `negative` on the maths lesson 15 My Turn, which showed `−3` and never the word the card had just taught, and method words on other maths My Turns. That is the gap the teacher named. `design-review-packet.py` now prints each vocabulary slide's script in the class view, so the reviewer's voice sweep hears the words the teacher says in front of the cards; before this it never could.
- **The second and third readers, on the change.** A fresh agent compared the old and new text row by row and found six things: the new check refused a picture's printed caption and a word's natural forms; the designer's new pointer had dropped `a card, or one explicit taught line` and the teacher's deferral exception; the contents line still said three to five; the designer had lost its pointer to A Picture Beside a Word for word banks and sorting sets; one story was removed from one file and left in its twin; and the pins could miss a deleted sentence or a paragraph moved out of its section. It also found small softenings (a `must not` become a description, `elsewhere` narrowed to the Teach slide, the reused-photograph instruction become a description). A second fresh agent then re-tested the repairs and found that the wider word matcher had lost the plural of words like `valley` and `key` (a regression, now fixed), that only one caption phrasing was heard, that one history sentence had been reworded without a decision (now restored), and that ten of its seventeen attacks on the pins got through. All of it was repaired, and all seventeen attacks are now caught. One small thing is left as it was: the classroom example the designer's copy of the timing rule carried repeats the continuity example the kept rule already has.
- **Pinned.** `scripts/tests/vocabulary_ledger_pins.json` holds every ledger row in its current home and its section, every changed rule's whole paragraph, the vocabulary homes paragraph by paragraph in their exact order, and every phrase the teacher retired; `test_vocabulary_ledger_is_kept.py` fails if a rule is dropped, reworded, moved out of its section, or brought back anywhere in the instructions, if the vocabulary home gains, loses or reorders a paragraph, and, in the development checkout, if the pins stop matching the ledger. Three existing tests pinned the designer's own copies and now look in the one home; a fourth pinned the old script-counts behaviour and now pins the teacher's rule, with tests for a printed caption, descriptions that print nothing, word forms, and every check reading the same slide.

**Not done yet, and named.** The wall and slide pulls (the wall's two-sheet arithmetic, retiring chip cards, green in success criteria, splitting a vocabulary slide whose pictures do not fit, one list of when a definition may run to two sentences) wait for those topics, as the teacher agreed. The surviving "named as a check" clause in `task-contrasts.md` waits for the quick-check topic. Behaviour is not yet validated on a real run: the before-and-after lessons the teacher reads are the acceptance test.

## 2026-09-23 - The Teach then Do rhythm is written once, and counts ideas rather than slides (4.2.285)

*Topic 3 of the streamline in `plans/streamline-plan.md`: the Teach then Do rhythm.*

- **The inventory.** Every place the plugin says anything about the rhythm was listed in its own words: 309 places in 29 files (`plans/2026-09-22-teach-then-do-ledger.md`). The core rules were written many times over ("a question to the room is not a Do" about 15 times, "one idea per Teach, then children use it" about 17), the designer's own rhythm section restated most of `preferences.md` in shorthand, and the content route a third time. An independent agent found 35 missed rules and 17 "copies" that carried a condition of their own; each was checked and added.
- **The teacher's decisions**, all twelve in one message and a second round on the read-back, are in the ledger in his words. The ones a lesson will feel: the rhythm counts ideas, not slides, so two teacher slides carrying one idea are fine and a second new idea before children have used the first is not ("you teach children something and they do something immediately with that information before you teach them more things. It's cognitive overload, 101."); setting the scene is for making the topic familiar, is never a beat with a made-up task after it, and is judged by his random-slide test (open any slide and see what it is doing and why, or go back one or two slides and see why); a discovery lesson may discover two things, from one exploration or a second that builds on the first; a question as the Do says in the task how every child commits, and gathering stays the teacher's; a sort and a short reason for its placements are one beat; a practical lesson opens on the challenge only under the bounded-attempt conditions, and a maths skill lesson opens with the model; the reviewer reads the rhythm on every review.
- **One home.** `preferences.md` → The Teach → Do → Teach → Do Rhythm holds each rule once, carrying what the designer's copy held alone: the route details, the purpose sentence, every beat earning its place with the three limits on cutting (a brief fact, enrichment, never thin teaching), and the three homes for a second job (from the task route). The designer's section keeps how it records `unlocks`, `minutes` and `thinking`, the launch sentence and the three-decisions paragraph, and now opens by sending it to the one home, naming what that holds. The content route's copy became pointers. The voice guide's practical shape moved in with its conditions.
- **Where the reviewer reads.** The skill route's cycle rules (cycles end with their own Your Turn, nothing between a My Turn and its Your Turn, the short cycle for a missing step, the `teach` and `practise` beats, one My Turn per move) sat below `## Output Format Block`, where the reviewer's reading stops; they moved above it word for word, under `## Cycles, and the beats around them`, and the vocabulary pins that name that section moved with them. The task route's staged-task rule moved the same way. The rhythm joined the reviewer's always-read list, which retires two triggers the reviewer could only meet after finding the fault.
- **Out of date or written for a maintainer.** "The rhythm section ... is short" (it is about 32 KB), "after all Teach/Do pairs", the practice-last summaries, "CURRENT" three times, a pointer to a section that does not exist, "refuses two My Turn units in a row for the same concept" (it refuses any two), and which `teach-needed` unit the validator checks.
- **Checked (code).** `validate-lesson-design.py` and `lesson-design-scaffold.py` accept a discovery lesson's further findings (any number, each taught and used before the next; the teacher asked for two) as `teach-why, use-learning` or `explore, make-sense, teach-why, use-learning` before `finish`, and still refuse two explanations before either is used; a test holds both programs to the same answer on ten shapes. `design-review-packet.py` reads the rhythm every review; its always-read note keeps the three-Teach procedure, the restating Do and the question to the room.
- **Tests moved, and two retired with what they tested.** Fifteen existing test functions in seven files pinned the designer's folded copies, the old triggers, the old discovery message or wording the decisions changed. Thirteen now pin the rule where it lives, and that the designer or reviewer is sent there; two tested a reviewer trigger that no longer exists (the section is read every review) and went with it. `teach_then_do_ledger_pins.json` and `test_teach_then_do_ledger_is_kept.py` pin every ledger row in its section, every changed rule's whole paragraph, the two homes paragraph by paragraph, and every retired phrase; `ledger_pin_checks.py` holds the checks the later topics will reuse.
- **Size, honestly.** The designer's own file is 2.2 KB smaller and its rhythm section went from about 10 KB to 7.5 KB. The instruction files as a whole are about 4.4 KB larger: the teacher's decisions added text that was never written anywhere (two slides and one idea, the random-slide test in his words, a question as the Do, two discoveries, the practical shape's conditions), and the designer's shorthand became full sentences when it moved into `preferences.md`. The saving this topic gives is one copy of each rule instead of three, not bytes. The reviewer now reads the whole section, about 32 KB, on every review, where before it opened only on a trigger; that was the teacher's decision, taken on the promise that the fold would shorten the section first, which it did not.

- **The second reader, on the change.** A fresh agent compared all 66 changed rows with their old words and found nothing lost outright, and seven things softened or narrowed: the scene's own slide was promised "when that makes sense" though the Slide Designer splits a Teach only when its board is too full (both copies now say that; whether the designer should be able to ask for the split on a board that fits is the teacher's question, not yet answered); the practical-lesson paragraph read as ruling out investigate-first lessons and every maths lesson (it now names an attempt at the target, and a maths skill lesson); "short" and "manageable" and "explore before Teach why" had fallen out of the fold; one story became no example where its neighbours kept theirs; hot-seating's commitment was looser than the decision; and two small descriptions. Its 26 attempts to break the pins found 12 that nothing caught, the worst being a whole section moved back below the reviewer's reading line, which would silently undo decision 6. Pins now record which route rules sit above that line and fail if one moves below it; the paragraphs the new rules point at (the bounded-attempt conditions, the maths opening, the one-warm-up limit, the synthesis rule) are pinned whole; a retired phrase is looked for in its own program as well as the instructions; and `CURRENT allows` is pinned as gone. A second fresh agent then re-tested the repairs and attacked the pins 38 times. The repairs held and broke nothing; it found the reviewer's own copy of its reading line unpinned, the scaffold's discovery gate untested on the shapes it should accept, two repairs with no pin, and pins that held a whole paragraph's words but not the paragraph itself. All of those are now pinned or tested: a paragraph pinned whole must still be exactly that paragraph, and the reviewer's line is found by its heading. What the pins still cannot catch is left to review: a new contradicting paragraph outside the rhythm's homes, a retired rule back in new words, a paragraph reordered within its section, and a section this list never recorded moving below the reviewer's line.
- **Stories kept here as they left the runtime.** A Year 4 class that could round 43 could not find the tens either side of 3,998 (17 September 2026): the deck went straight into rounding and left the step to a teacher who would notice, and a cover teacher did not. The repair taught the next day was a My Turn on two cases (`528` and `5,996`: make the ones digit 0 for the ten below, add 10 for the ten above, with `?` at each end of the line) and a Your Turn on four more, about four minutes before any rounding. On the same day a skill-lesson `teach` slide that set `43 children came to the fair` and `about 40 children` in two text cards around a small number line was the right idea and read as wordy. A Year 4 history class (15 September 2026) could have written its explanations after the second chunk and instead sat through three more short whiteboard beats on the carpet, because the content route put the main work last and the guidance that says to treat a substantial beat as main practice had nowhere earlier to put it; that is why a Practise now sits where the class is ready for it. And a teacher on a finished Year 4 history deck (18 September 2026) said "because we had a big task already, then we've also got to do worksheet, it won't fit the 45 min.", which is why the designer writes honest minutes and the validator adds them up.
- **Maintainer history kept here as it left the runtime.** The rhythm paragraph in `preferences.md` carried a bracket for whoever maintained it: the skill route's cycle grammar (every My Turn plus Our Turn cycle ends with its own Your Turn) was added because it was previously possible to satisfy the route while breaking the rhythm sentence. The bracket now names only the grammar.

**Not done yet, and named.** Behaviour is untried on a real run; the before-and-after lessons the teacher reads are the acceptance test. The three questions the change raised were put to the teacher and answered: the scene stays the opening of the first Teach, on its own slide only when the first board would be too full ("sometimes it doesnt always have to be a full slide, sometimes just a note on slide is fine"); a discovery lesson may discover any number of things, each taught and used before the next; and the reviewer keeps reading the whole rhythm section every review. The quick-check paragraphs inside this section change with the next topic (4.2.286); four corners and the time numbers wait for the topics that own them.

## 2026-09-23 - A quick check is a fresh case, written once, and the reviewer sees every pupil beat beside its teaching (4.2.286)

*Topic 4 of the streamline in `plans/streamline-plan.md`: quick checks.*

- **The inventory.** Every place the plugin says what makes a check genuine was listed: 307 places in 32 files (`plans/2026-09-22-quick-checks-ledger.md`). The fresh-case rule was written about twenty-five times; the one sentence that says what makes a case fresh sat in the worksheet guidance; and the "name it as a check" excuse that 4.2.283 meant to remove survived in five places, beside nine honest rules that also tell the designer to say what a beat is for. An independent agent checked the list before it went to the teacher.
- **The teacher's decisions**, all ten agreed in one message. The "named as a check" and "named as one" clauses went, and one sentence beside the rule now says which "say what it is for" rules are honest: the ones that change what the beat claims or what is done to the task, never a label standing in for the fresh case (after teaching the key on a map of Brazil, the check reads the key on a map of Kenya). The good/bad apprentice sort is no longer allowed as "a two-minute orientation" in the contrasts file, the reviewer or the history file; each still says such a sort is not evidence. A label check straight after the Teach uses a new picture of the same thing, a blank copy of the same one where no other exists (the world map), and the same picture in a later lesson, when it is retrieval. A fact, name or definition may be recalled straight after teaching once the answer is off the board, and claims recall, never understanding; a check that an idea has landed is a fresh case (after the enamel slide: `cover the board: what is the hard outer layer called?` is recall, picking the Teach's own sentence is finding, `which of these three teeth has lost its enamel?` is the fresh case). Eight catalogue formats that give back what was just taught (two summaries, a headline, two things, teach your partner, three-two-one, a quick sketch, a sketchnote) keep their place with a one-line limit; Two Truths and a Lie takes True or False's; Diagram to Words, Explain One Link and Words to Diagram are for a diagram or link the class has not had explained. PSHE's and RE's own wording stays. His rebuilt Tudor lesson stays as the calibration, without its date.
- **One home.** `preferences.md` → `A quick check is a fresh case, not the last slide again` holds the rule, with what makes a case fresh (carried from the worksheet guidance, which keeps its own copy), the recall line and the honest-say-so line beside it. The designer keeps the disagreement case and a pointer; the reviewer keeps its own sentences; every permission for a short recall keeps its words and gains a pointer that carries both halves of the recall line (a fact, name or definition may be recalled with the answer off the board; an idea needs a fresh case), except the catalogue's sort permission, whose pointer carries the sort's own condition (the cards are cases the Teach did not show).
- **Checked (code).** The review view's `Each Do beside the teaching before it` read only a content lesson's Do straight after a Teach, printed a sort's answer as `(none written)`, and never counted the Teach's takeaway. It now shows the first beat where children use each piece of teaching, in every route, beside everything the class met since the last pupil beat: a Do or Practise beside its Teach, every Use the learning beside its Teach why and the result made visible, an Our Turn beside its My Turn and a Your Turn beside its whole cycle, every Talk beside its own Stimulus and any grounding input, a combined stimulus and talk beside the grounding before it, a planning checkpoint, and an enabling input's own instruction (never counted against its own answer) and the task after it. A Practise that follows a Do is not shown again, because the teaching it follows was already read beside that Do. It reads sort and evidence-classification answers and a Talk's expected positions, shows an Our Turn's question, and counts the takeaway, including a sticky fact. On the 52 filled saved designs it builds without error, loses nothing the old section showed, and no longer prints a blank question. The trigger it needed is the always-read rhythm section (4.2.285). The reviewer's sample cases no longer teach the excuse.
- **Pinned.** `quick_checks_ledger_pins.json` and `test_quick_checks_ledger_is_kept.py` pin every row, with the excuse's wordings barred. Nine rows of the Teach then Do ledger whose words this change altered are recorded in its pins with the decision that changed them, and five more whose whole-paragraph pins now carry an edit made beside them say so.
- **Size, honestly.** The instruction files are about 6.3 KB larger, and the review packet's code about 6.4 KB: the limits on eight formats, the recall and honest-say-so lines and what makes a case fresh are new words, and the pointers that replaced repeats are short. The gain here is that the rule and its limits are said once, beside each other, and the excuse is gone from everywhere it survived.
- **The second reader, on the change.** A fresh agent compared the 41 changed rows with their old words, ran the new view over every saved design and attacked the pins 42 times. Nothing was lost outright. It found: the short recall pointers carried only the half of the recall line that permits (so `cover the board: why does a shadow form?` passed them, though the home sends an idea to a fresh case), now each carries both halves; Two Truths and a Lie had taken half of True or False's limit, now all of it; the home had lost A21's "do not force different answers" and gained an absolute ("the one thing recalled"), both repaired; a sentence permitting a sort of a different apprentice's deal had been added to the contrasts file without a decision, and it failed the same everyday-sense test, so it came out; the label formats lacked the world-map limit; the view counted an enabling input against its own answer and missed an Our Turn's question, later Talks and the task after an instructed input, all repaired and tested. Its 42 attacks found 14 that nothing caught: the short forms of the excuse are now barred ("named as a check" and "named as a quick check" everywhere; "two-minute orientation", "named as one" and "the design says so" only in the files they left, because each is ordinary English elsewhere and the last has an honest use in the vocabulary section), a retired phrase is looked for in the plugin's top-level Python programs as well as its instructions, the view's reach is tested shape by shape, and two sentences the rule leans on are pinned whole. What no pin catches is left to review: a new paragraph saying the opposite in new words, a retired phrase with a word inserted, emphasis added or split across two joined strings (a bar is a literal string), a builder JavaScript file, a deleted test assertion, and a catalogue entry moved whole with its heading.
...
**Not done yet, and named.** Behaviour is untried on a real run. Three questions the change raised were put to the teacher and agreed: a label-reading check after a new map skill is on a map the lesson has not shown; the history contrast's simpler task that is right is the quick match of jobs the slide did not show to what each gave the family; and the summary formats' best-use lines, Quick Sketch's opening (now a sketch from memory of something taught earlier) and Diagram to Words' copied-picture clause (now in a later lesson) name the moment their limits allow. Out-of-date catalogue text the ledger listed (the "answer-scatter" name, "Brain Dump", the section counts, a quoted reviewer phrase) waits for the topic that folds the catalogue; so do a sort item's `detail`, which the view does not print, and the stick-in pedagogy's "a genuine retrieval task" for the blank world map, which the new label rule calls the check.

- **Stories kept here.** A Year 4 RE beat asked children to finish `My walk matters because...` under a speech bubble that already said `being outside helps me feel calm`; the thinking line for that beat, honestly written, was `Where on the slide is the reason?`. It is the twin of the Zara beat recorded under 4.2.123, and it stays in `lesson-designer.md` as a plain example. A Year 4 PSHE quick check asked what Sophie's body uses energy for when she is sitting still; the answer, breathing, lived only in the script of the slide before, which is why the reviewer treats a check whose answer lives only in a script as unprepared. That story had no entry here; it is copied, not retired, because `preferences.md` still tells it where another topic owns it.
