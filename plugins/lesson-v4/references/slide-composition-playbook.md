...
**Stage new information when it helps children follow the teaching.** Establishing and asking can share a slide when the example, explanation and question remain easy to follow. Split when they represent distinct teaching moments or when their useful readable size requires it. Before splitting a practice set, try the available width at a sensible shared scale; children should see the whole current set where it fits. Each resulting slide must make its current teaching or pupil action clear. Preserve all required content across the sequence.
