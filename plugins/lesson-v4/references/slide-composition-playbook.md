...
**Dividing one My Turn is for a representation that cannot be read shared, and for nothing else.** The default is still both examples on one slide, because a teacher modelling two numbers side by side can point from one to the other. Divide when the drawing has a size below which it stops working and sharing the slide would take it there: two four-column place-value charts on one slide give each column 0.6in and each counter a tenth of an inch, which the build refuses outright (`PLACE_VALUE_COUNTERS_TOO_SMALL`), and one chart on the same slide gets double the width and counters at full size. This is not a licence to divide a My Turn that merely feels full: text, a question set and a photograph all read perfectly well beside a twin, and the reason to divide has to be a measured floor the shared version breaks, named in the build.
