...
Vocabulary is presented on one `key-vocabulary` slide, the entry the design planned in `vocabularyIntroductions`: every retained word with its definition and visual, straight after the last slide of the unit the entry names. The card is the reference children glance back at, and a reference is one place; the teaching of each word happens inside the beat that needs it, where the Teach's own landed sentence says the word in vocabulary green on the thing the class has just seen. So a deck never stops a second time for a glossary, and a word is never split off onto its own slide because it arrives later: it is on the card, and it is taught in its beat. The design owns where the one slide sits; do not move it.
