...
**A slide where children now do something should not look like a slide where the teacher was explaining.** The lesson's beats already differ in kind - teacher input, a source to investigate, paired talk, independent work - and a deck where all four arrive as similarly dense cards makes the teacher read every slide to find out which one they are on. Flicking through it should feel like the lesson, not like a document about the lesson. What carries that is what leads the slide and how much of it that thing takes: on a Teach the thing being taught is the largest object and the task line is small or absent; on a Do the pupil's question or task leads and the material is the field it acts on; on independent work the material children read from takes most of the board and the teaching sentences are gone, because they have already been taught. This is a difference in composition, and it is not made with badges, banners, a progress strip or any other furniture announcing what kind of slide this is - the change in what dominates is the signal, and adding a label is how a deck ends up explaining its own structure to the teacher instead of showing it.
...
**Ask what children are looking at while you say this.** The rules above set one thing above another, two at a time. They do not limit how many things are on the slide at all, and a slide can obey every one of them and still be a page: nine separate text objects, each correctly sized against its neighbour, with no answer to the only question a teacher actually has at the board. So ask it directly of a teaching slide: while I am explaining this, what are the children looking at? There should be one obvious answer - the source, the model, the example, the comparison, the key sentence - and everything else on the slide should be something a child's eye passes over on the way to it, or arrives at afterwards.

Two ways this goes wrong, and they look nothing alike. The first is the pile: a rule, a definition, a source, a question, a timeline and the criteria, all present, all legible, all competing, and the teacher has nowhere to point. The second is the one that gets reached for as the cure and is worse: a picture and `What do you notice?`, with the actual teaching moved into the speaker notes where no child sees it and no teacher reads it mid-lesson. **The target is not less teaching on the board. It is less arriving at once.** A slide carrying one teaching sentence and one strong example is doing more work than a slide carrying five supporting blocks, and neither of them is a slide carrying nothing.

The repair is almost always staging rather than deletion, and §8 owns it: the explanation crosses two slides in the order a teacher would say it, and the object it is about goes on the slide with the sentence that teaches it, not the one after. A slide of three explanation sentences and a timeline, with the portrait those sentences describe sitting on the next slide, has split at the page boundary rather than at the teaching one.

**The board does not reprint what is already in the child's hands, and it says whose sheet the work is on.** When a practice beat's questions, source or frame are printed on the child's own sheet, the launch slide carries the task in words, the success criteria and anything children genuinely cannot have in front of them, and it names the sheet in its title or first line. A Year 4 history deck printed all three of the worksheet's questions on the board beneath the line `answer the three questions on your sheet`, which spends the largest part of the slide on the one thing every child is holding. The cost is worse than space: a cover teacher reading that board takes the questions as the thing to work through with the class, does it, and then hands out a sheet asking the same three questions, so the independent task the lesson's evidence depends on has already been answered together. So the slide that launches a printed task says so in words a stranger reads correctly. Its title names the moment (`Worksheet time`, `Now your independent work`) with the task after it, and its first line begins on the sheet (`On your worksheet: her account and the three questions`). Say it, rather than marking it: the sheets themselves print a small book or pencil beside the level code, and a cover teacher has never been told what that symbol means, so a symbol is a signal only to the person who already knows. The criteria stay on the board because children consult them while they write, which is also why the sheet does not reprint them. The limit is material children need while working that the sheet does not carry, such as a word bank, a worked example they refer back to or a diagram too large to print: that stays on the board.
...
Judge support at its actual size alongside the task. A quieter panel must still
be readable; a complete panel must not make the source, example or response
surface unusably small. Try another arrangement before reducing type or adding
a slide. When the reference is itself the object being taught, it leads and
deserves the larger share. For projected tables the renderer enforces a 20 pt
minimum and shared header/body sizing; needing less is a composition fault,
not permission to shrink. Preserve the information and the support children
need, then change the arrangement or stage the dependent work coherently.
...
**Stage new information when it helps children follow the teaching.** Establishing and asking can share a slide when the example, explanation and question remain easy to follow. Split when they represent distinct teaching moments or when their useful readable size requires it.

**A Teach beat splits where its teaching turns, and both halves are still teaching.** The route a Teach board carries runs from what the class already has, through the new thing, to the look at the picture and the sentence that lands (`preferences.md` → Slide Philosophy, `The fact is the destination`). When it will not fit one board at the ceiling, cut it at the turn: the scene set on the first slide (the lead and the line that changes it, beside the picture), the look and the landed sentence on the second (the question, the line it needs, the star fact, beside the same picture). Never cut it at the page boundary. On 14 September 2026 a repair cut a Year 4 history beat into a photograph with the label `A Tudor farm household` and, on the next slide, every word with no picture and no notes; the teacher met the first half and had nothing to teach from, which is the failure the check now refuses (`TEACH_SPLIT_LEAVES_A_LABEL`, `TEACH_SLIDE_WITHOUT_ITS_SCRIPT`). The picture stays on both halves when the words are about it; the script is cut where the slides cut; the words are never shrunk or shortened to avoid the split, because a board of small text is the wall again and a clipped line is a label. The set the user chose is `references/examples/tudor-teach-slides.lesson.json`: one split per beat, each half at least a lead or a question with one line beside the picture. Before splitting a practice set, try the available width at a sensible shared scale; children should see the whole current set where it fits. Each resulting slide must make its current teaching or pupil action clear. Preserve all required content across the sequence.

A split must also survive a teacher who has never seen the deck. Each continuation leads with its new explanation or action and retains the evidence and support that action still needs. Repeat a picture, diagram or reference on both slides when children still use it on both; repeated evidence is not redundant teaching. Keep pronouns, comparisons and instructions attached to their visible referents. A reference on a previous slide is not accessible during independent work unless another usable copy is actually available. Match necessary speaker notes to each physical teaching slide, without adding scripts to routine answer reveals. A slide labelled My Turn, Our Turn or Your Turn carries that turn's own question, task or answer, and the build refuses one that does not (`TURN_SLIDE_WITHOUT_ITS_TURN`). A My Turn is the one beat where a split has to prove what it is: two modelling slides in a row means the class watches two MOVES before practising either, so the build refuses a second consecutive My Turn slide from a different source unit (`MODELLING_RUNS_WITHOUT_A_TURN_FOR_THE_CLASS`). Two moves is the fault, and the source unit is the evidence. Examples of one move live in one My Turn unit, so two My Turn slides carrying the same unit are one modelling moment the layout had to divide, and the class still practises that one move next. Where the examples are genuinely different moves, the repair stays upstream in the teaching sequence (`teaching-sequence-skill-based.md`, 'Several examples of one move belong inside one My Turn unit'), not here.

**Dividing one My Turn is for a representation that cannot be used shared, and for nothing else.** The default is still both examples on one slide, because a teacher modelling two numbers side by side can point from one to the other. Divide when the drawing has a size below which it stops doing its job and sharing the slide would take it there. Two four-column place-value charts on one slide give each column around 0.6in, and the build refuses that twice over: `PLACE_VALUE_COUNTERS_TOO_SMALL` where the chart carries counters, and `PLACE_VALUE_WRITE_IN_TOO_NARROW` where a row is left blank for the teacher or the class to fill in live. One chart on the same slide takes the column past double the width, and both floors clear easily.

Ask what the representation is FOR, not only whether it can be read. A chart of printed digits is read, and it survives being shared. The same chart with an empty row is written in, and a column wide enough to print a digit is not wide enough to write a place value in by hand - the difference is invisible on screen, which is why this one keeps being missed. Two decks shipped a day apart with charts a teacher could not write in (flagged by the user, 4 September 2026: "there's no way the teacher if they wanted to could write neatly in the columns because they're not wide enough ... I would have instead still used both but on 2 different slides, so each get more space"). Where a chart is only ever going to be read, print its digits, or ask for the bare heading strip with `"rows": []`, rather than leaving cells blank and claiming the width a written answer needs.

This is not a licence to divide a My Turn that merely feels full: text, a question set and a photograph all read perfectly well beside a twin, and the reason to divide has to be a measured floor the shared version breaks, named in the build.

Success criteria follow the same rule as any other shared reference, and this is where that gets forgotten, because the criteria feel too important to leave off. Put them on the slides where children are actually working to them. Where they belong, they stay complete and exact - their appearance on an earlier slide is never a reason to shorten or compact them - and they take a receded share, per §5. What they must not do is arrive on every part of a split by default: a Year 4 history lesson referenced its criteria on three source units and printed the panel on six slides, so on the two continuation slides the green box was as loud as the new source comparison it was meant to serve. Place a repeated panel beside the task (a row) rather than as a full-width band beneath it when the stack is tight, and keep it the smallest column in that row: growing a bottom panel's share squeezes the very task the criteria serve, and giving it a third of a row beside two photographs children must inspect does the same thing sideways.

---

## 7. Empty space is working space

Before composing a live model, identify the marks the teacher will add and give those marks a usable home. A filled diagram is not working space for a new equation or explanation. Keep the support, but arrange it around the demonstrated work. An off-slide physical demonstration needs no on-slide writing area. When a slide leaves room for the teacher to complete a diagram live, simply leave the room. Never label it `Working space`, never add `teacher writes here`, and never use rows of typed underscores as a substitute.
...
One worked example belongs on one modelling slide when each example needs its own large annotated visual. Two examples that each own a place-value chart, number line, bar model or labelled diagram should not share one helper and overwrite each other. What it costs the teacher is the reason: they model the first example on the figure, then stand at the board rubbing it all out before they can start the second, while the class waits. Each modelled example gets its own slide, with the same title and the same source unit, so the next click gives a clean figure (the build refuses `ONE_MODEL_PER_ANNOTATED_FIGURE`). A Your Turn where children draw their own figures, and a reference figure nobody writes on, share a slide as before. Off-slide calculations with no individual on-slide visual may share a slide when the source unit and readable geometry allow it.
...
**When children used a drawn tool to reach their answers, the answer slide keeps a blank copy of it.** The answers are shown in green as usual, and a blank number line, bar model or place-value chart like the one children drew sits beside or below them, so the teacher can go through a wrong answer by modelling it again rather than reading the right one out. The teacher asked for exactly this after teaching a Year 4 rounding deck whose answer slides were text (17 September 2026). The limit: the blank copy is for going through independent work, so a starter or recall check with no tool gets none, and where the finished tool is itself the answer (a completed sort, a marked map, a finished model after a My Turn) it is shown finished instead.
...
Parallel peers share geometry as well as purpose. A row of category or option cards uses equal card height, equal vertical position, one shared maximum safe text size and one alignment, and that alignment is **centred**: a category name or a sort destination is a label, not a line of prose, and a text object left at its default left alignment reads as a sentence that has lost its place (the user, on two `Helped him straight away` / `Helped him when he grew up` cards, 14 September 2026: "they weren't centred... because these are category cards, they should be centred"). Set `"align": "center"` on each. A short closing statement may use a centred content-width card when a full-width card would contain unused white space.

**Cards that share one text size are only as big as the longest of them.** A column or row of cards settles on one size (`sizeGroup`), which is right, and it means one long card holds every short card down with it: a Year 4 slide carried `If you needed wood, somebody had to fetch it. Wheat had to be carried in, and the animals had to be fed.` beside `Look at her. She isn't being paid to do this.`, and all three cards printed at 19pt with the short ones sitting in white space (the user: "some things in these text boxes could have been made bigger"). Before accepting a group near the 18pt floor, look for the card that is setting it: split it into two cards where the layout has room for another (the two sentences above became two cards and the group rose to 23pt), or move a sentence to the card beside it, so the cards are near one length. The words stay; their grouping changes.
...
**A caption costs the picture its height, so it earns its line once.** A caption takes a band out of the picture's cell, and a contained picture shrinks to the space left. A Year 4 history deck (14 September 2026) printed `Tudor harvest work, reconstructed` or its like under every picture on every slide, including a picture's third appearance and every Teach slide whose script already said what the picture was; the user: "in the teach slides it didn't actually need these, and if these were taken away the pictures could have been bigger, which would have helped visibility." So: no caption on a Teach slide whose lines or script already say what the picture is; no caption on a picture the class has already met with one; and a caption that must be honest about what a picture is (a reconstruction, a modern photograph of an old place) is said once, at the picture's first appearance or in the script, not reprinted beneath each return. The limit is a caption that carries something a child needs and nothing else on the slide says: a source's date and maker, a place's name, which of two pictures is which.
...
Vocabulary is presented on `key-vocabulary` slides, one per introduction the design planned in `vocabularyIntroductions`. Each slide carries that introduction's words with their definitions and visuals, and sits straight after the last slide of the unit its entry names. Two entries anchored to the same unit follow in the order the design lists them. So a lesson that introduces all its words together still has exactly one vocabulary slide; a lesson that needs one word before an instruction and a contrasting pair after the beat that gives them meaning has two, in those two places. The design owns the timing: do not merge its groups into one slide to tidy the deck, and do not split a group because the words look separable.

What a run of single-word slides at the open of a lesson does is scatter into four slides a reference children wanted in one place, none of them near the teaching that needs it. That is a reason to question a design that plans four separate introductions before anything has happened, not a reason to override one that introduces two words at the point they are used. When one word needs teaching rather than introducing, that teaching is a Teach beat with its own slide in the sequence, in addition to - never instead of - the word's card.
...
**Compose the explanation's lines as separate pieces, not as one block, and build the slide from a named teach layout.** Each line is its own short statement doing its own job, so a slab of them in one card is three sentences a child reads as a paragraph and a teacher reads out in one breath. A Teach unit's slide is `template: "teach-layout"` with a `layout` from the catalogue in `templates.md` → `teach-layout`: you supply the lead, the lines, the question, the line to remember and the pictures, and the builder arranges them, text centred, cards that belong together the same size with one text size and even gaps. That is the teacher's standard for every slide (13 September 2026: "align centre is our friend, we align centre with elements too, we make sure things are spaced same width and height in different elements too"), and it is the builder's job because a designer composing free zones by hand does not reliably meet it. Free zones are not the route for a Teach unit's slide, and the check refuses one (`TEACH_SLIDE_NEEDS_TEACH_LAYOUT`): assembled by hand, every Teach slide in a Year 4 RE deck came out as a picture on one half and a column of equal cards on the other, the arrangement that is cheapest to assemble, the day after the teacher had asked for teaching placed where it belongs.

`references/examples/tudor-teach-slides.lesson.json` is a whole Teach run the user chose from four built versions (14 September 2026): five slides for three beats, four layouts, each beat's route in whole sentences, the split where the teaching turns. Open it before composing the first Teach slide of a knowledge lesson, because it is the shape being asked for and prose about it is the lossy version.

**Choose the layout from what the slide holds, then from what each line is about.** Count what you have: how many pictures, how many lines, whether the slide opens on a question, carries a claim to test, compares two things, runs in order or gives a source to read closely. That narrows the catalogue to a handful. Choose among those by the lines: a line describing the whole thing goes across the top (`lead-picture-lines`, `big-fact-picture`); a line about one part sits next to that part (`labelled-picture-lines`, `two-pictures-captions`, `three-pictures-captions`); what happens next runs in order (`picture-steps`, `zigzag`); and a slide that opens on a question puts it first (`question-lines-picture`, `question-picture-answer`), which is a slide whose unit keeps its sentence for the end or the later half of a split, never the first slide of a unit that has a `headline`. A Teach slide with no picture is an ordinary Teach slide and has layouts of its own. What is refused is the default that produced both complaints: every line concatenated into one black card because they arrived in one field, and one arrangement repeated because it was the first that fitted.

**The next Teach slide takes a different layout.** Read the Teach slides as a run before choosing any of them, as the role's first step asks, and vary the arrangement along it. More than one layout fits almost any set of words and pictures, and the check refuses two consecutive Teach slides that share one unless they carry the same unit (`TEACH_LAYOUT_REPEATED`). Variety serves the slide rather than novelty: choose the arrangement that best shows this slide's relationship, and when that matches the Teach slide before, take the next best.

Spacing and position carry most of the separation, and one line of the explanation may also be orange when it is the line carrying the weight: on a teach layout, set `"orange": true` on that line. `teacher-slide-visual-profile.md` → Semantic colour owns that, including its three bounds: one line per slide, not on every Teach slide, and never the sticky line or a question. Reach for layout first and let the colour be the thing you add when layout has not done it. It is not a paragraph and not a caption. A Practise or Do-the-task unit's `launch` takes a slide of its own before the task slide, and the `goodLooksLike` pair takes `template: "strong-and-weak"` (`templates.md` → `strong-and-weak`): the good instance and the weak one in two cards the builder marks with a tick and a cross, the weak card red throughout, the `difference` as the one line underneath, and the `established` line above it only when the design still carries one. The `steps` follow as a numbered blue list, and the task slide then carries the instruction.

The `established` line leads, on a short slide of its own before the pair, and when it sets a case that slide carries who it is, what happened and what they are being asked - the model means nothing to a child with no question in mind. Colour does the reading for them there: what is generally true stays black, the case itself takes supplied orange, and the picture of the person goes beside the case rather than beside the general line or the instruction (`teacher-slide-visual-profile.md` → Semantic colour, Task-to-object attachment). An invented child needs showing more than a real one does, so a face on that slide is worth its room. `rehearsal` takes its own short slide between the pair and the task, and `reasoningWords` go on that same slide as a `chip-bank` underneath, because the moment a child needs `so` and `because` is the moment they are saying it and about to write it. Three things on it and no more: `sayIt` as the instruction, `partnerAsks` as the partner's question in house blue, and the words. **The routine around them stays in the script.** The teacher cut `Then your partner asks you one question:` and `Say it again with that bit added. Then write it.` from that slide on 18 September 2026, and both were already in the notes word for word: they narrate who does what when, which a child does not look at, and they were pushing down the three things a child does look at. The order is the point - a child says the explanation, is asked one question about it, and only then writes, so what they write is the second version rather than the first (`explanation-tasks.md`). Do not assemble the pair by hand from a `row` of two `text` cards with `Strong:` and `Weak:` typed inside the sentences: that is what three Year 4 decks each invented separately, on a different side each time, and the check refuses it by name (`LAUNCH_PAIR_NEEDS_ITS_TEMPLATE`). Notes may enrich it but do not hold a hidden second lesson. When a title asks a question, the answer idea is visible on the slide. Keep every structured `keyQuestions` entry visible in a readable treatment. A pupil-action slide makes the material, decision, response and active reference visible. When a non-null pupil instruction is not independently actionable, preserve it and return `PUPIL_INSTRUCTION_AMBIGUOUS` instead of repairing it through presentation wording.
