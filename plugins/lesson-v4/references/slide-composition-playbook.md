...
**Compose the explanation's lines as separate pieces, not as one block.** Each line is its own short statement doing its own job, so a slab of them in one card is three sentences a child reads as a paragraph and a teacher reads out in one breath. Give each line room and let them sit where each one belongs. Three shapes that work, all of them ordinary compositions: the lines stacked as separate cards down one side with real space between them; the first line lifted to a full-width statement across the top with the rest beside the picture; or the lines placed against what each describes, one above the figure, one beside it, one under it. The teacher's own mock-ups of a science slide used all three (12 September 2026). Choose by what each line is about: a line describing the whole thing goes above or across, a line about one part sits next to that part, a line about what happens next sits under. What is refused is the default that produced the complaint, every line of the explanation concatenated into one black card because they arrived in one field.

Spacing and position carry most of the separation, and one line of the explanation may also be orange when it is the line carrying the weight. `teacher-slide-visual-profile.md` → Semantic colour owns that, including its three bounds: one line per slide, not on every Teach slide, and never the sticky line or a question. Reach for layout first and let the colour be the thing you add when layout has not done it. It is not a paragraph and not a caption. A Practise or Do-the-task unit's `launch` takes a slide of its own before the task slide: the `established` line first in black, the `goodLooksLike` pair as two separate items with the difference visible, and the `steps` as a numbered blue list; the task slide then carries the instruction. Notes may enrich it but do not hold a hidden second lesson. When a title asks a question, the answer idea is visible on the slide. Keep every structured `keyQuestions` entry visible in a readable treatment. A pupil-action slide makes the material, decision, response and active reference visible. When a non-null pupil instruction is not independently actionable, preserve it and return `PUPIL_INSTRUCTION_AMBIGUOUS` instead of repairing it through presentation wording.
