...
**`visual` field:** Any content object the deck can draw, the same object with the same fields it takes on any other slide. The card sizes the picture to itself: a compact figure (an angle, a clock, coins) gets a panel as wide as it draws beside the definition; a figure that reads along its length (a number line, a bar model, a fraction wall) gets a longer one; a picture made of words and parts (a table, a chart, a sort board, question cards, a timeline) goes under the word at the card's full width. Cards with a picture share the slide's spare height. Give it vocabulary-sized content: a picture of the word, not the task - two or three intervals on a number line, `{ "word": "interval", "definition": "...", "visual": { "type": "numberline", "start": 0, "end": 20, "interval": 10, "labels": "all", "highlight": { "from": 0, "to": 10 } } }`, one clock time, a four-row tally. A card never drops a picture it can draw. When two or more words on one slide each carry a full-width picture and together they cannot be read, the build stops with `VOCAB_PICTURES_TOO_BIG_FOR_ONE_SLIDE` naming them: give one a smaller picture. **Prefer built-in drawings over `image` when they carry the meaning - they require no picture sourcing and cannot become a missing-file placeholder.**
...
**A `fontSize` is where text starts, not where it ends.** The build shrinks it to whatever the box actually holds and stops at the 18pt projection floor, so the gap between the size you set and that floor is the only room the fit pass has to work in. Set a ceiling at or near the floor and there is no room at all: the spec check refuses it outright, and a line that fits at 17pt but not 18 takes the whole build down rather than settling a point smaller. Pick the size you would like the text to reach when the zone is generous, and let it come down on the slides where the zone is tight. Where text keeps arriving at the floor, the answer is a zone that holds it or fewer words in that zone, not a lower ceiling.
...
**A label can be words at a mark:** `{ "at": 2600, "text": "2,700" }` prints "2,700" under the 2,600 mark, so a "Has this line been completed correctly?" slide shows the mistake on the line itself instead of in a row of cards beside it. The `at` must be on a mark.
...
- `kind` is exactly `point` (a dot on a place), `line` (a river, a route, a border you are tracing), or `area` (a region, closed for you: a dashed outline by default, or washed with a soft translucent colour with `"shaded": true`).
...
- **`"shaded": true`** on an `area` fills it with a soft translucent wash of its own colour instead of outlining it, so the region reads as filled while the coastline and borders under it stay visible. Use it when the region *is* the thing being taught (where rainforests are, which countries a desert crosses); leave it off when you mean "somewhere in here". **When the mark is what the question turns on - "name the highlighted continent" - shade it.** A dashed outline is a quiet mark: on a map at anything less than full-slide width it is a thin line a class has to hunt for, and a starter whose first job is finding the thing being asked about has failed before the question starts. Shading survives being small; an outline does not. Hatched rather than solid on purpose, so the coastlines, rivers and borders underneath stay readable, and so it survives a grey photocopy. Shading works on every map and every surface, because the map is one drawing placed as a picture (it used to be refused on the plain slide map, which drew PowerPoint shapes).
...
`selectedCountry` accepts `Brazil` on the South America map. It fills Brazil while keeping its printed border, on every surface. `basin` accepts `Amazon basin` and draws an orange dashed boundary with a white halo, so the region cannot be mistaken for a solid national border. Both belong to `map: "south-america"` only, and are refused by name on any other map.
...
**One map, on every surface.** The board, the worksheet, the working wall and the stick-in pack all draw this one map from the same fields (`shared/visuals/map-svg.js`), so a map copied field for field from a slide onto a sheet, a wall card or a piece is the same picture. Anything that cannot print readably at the size it is given is refused by name: `MAP_LABELS_DO_NOT_FIT`, `MAP_ZONE_TOO_SHALLOW` (the caption, key or footer leave no height for the map), `MAP_TOO_NARROW`, `MAP_MARKERS_OVERLAP`, and `MAP_UNKNOWN` for a map name the package does not ship.
...
A staged teaching explanation of why the flat map looks the way it does: the world on a globe, the same geography turned to the Pacific with the cut line shown, then the equirectangular map opened flat with matching patterned Pacific edges and arrows showing those edges join. It explains the projection, so it cannot also carry `annotations`, `selectedCountry` or `basin`, and it requires `map: "world"`. On a sheet or the wall, where the page grows downwards, a narrow space puts the flat map on its own row under the two globes.
...
If the slide-designer assigns a content type to an incompatible zone, the builder refuses the pairing with `CONTENT_ZONE_INCOMPATIBLE` rather than replacing the requested content with text. The slide-designer should validate class fit before emitting the spec. One trap worth naming: `label-diagram` is the only *figure* that refuses E-narrow, so a labelled diagram in the narrow side of a split is refused rather than rendered as the literal text "[label-diagram]". It is not the only content type that refuses E-narrow: read the column, not that sentence.

**This table applies to content nested inside a container, against the container's zone.** A container draws its children inside its own box but passes its zone class down, because the class is a fact about how wide the slide is there and no panel can make a narrow strip wide. So `sc-panel` being ✓ in E-narrow does not make its `content` legal there: a criteria panel in a sidebar takes text or a list, and a `table` inside it is refused for the sidebar's width. A Year 4 PSHE deck put a two-column "The change / The reason" table into a sidebar panel on both of its task slides, spent three repair passes on other faults, and shipped both slides blank (21 September 2026). The refusal now names the container and the zones the type does fit; check a nested type's own row before nesting it.
