...
| `balanced-pattern-plate` | A neutral broad proportional food-group plate for Year 4: larger fruit-and-vegetable and starchy-carbohydrate areas, smaller protein and dairy-or-alternative areas, and a very small oils-and-spreads area. Water sits beside it; foods high in fat, salt or sugar have a separate "less often / small amounts" cue; the caption frames balance across a day or over time. `mode: "teaching"` labels every group with varied unbranded examples; `mode: "practice"` keeps the proportions but replaces groups not named in `givenGroups` with pupil-decision spaces. Never use calorie, weight-loss, moral, or bad-food labels. |
...
A broad proportional food-group pattern for Year 4. The two largest areas are fruit and vegetables and starchy carbohydrates; protein and dairy or alternatives are smaller; oils and spreads are visibly very small. Water is shown beside the plate. Foods high in fat, salt or sugar sit in a clearly separate cue labelled **less often / small amounts**, never as a moral category. The default caption is **"Aim for this balance across a day or over time — not every meal."**
...
**`mode`:** `"teaching"` (default) labels every area and prints varied unbranded examples; `"practice"` preserves the exact same sector proportions but replaces every group not listed in `givenGroups` with a large dashed decision space. **`givenGroups`:** any of `fruit-vegetables`, `starchy-carbohydrates`, `protein`, `dairy-alternatives`, `oils-spreads`; the practice default gives the two large areas. **`groupLabels`:** optional map from group key to a short teaching label. **`examples`:** optional map from group key to a short array of unbranded foods; defaults include apple, carrot, peas, berries, potato, rice, pasta, bread, beans, eggs, fish, lentils, milk, yoghurt and fortified soya. **`water`:** set `false` only when water is taught elsewhere on the same slide. **`caption`:** overrides the across-a-day wording; an empty string hides it. **`lessOftenLabel`:** overrides the descriptive heading on the separate less-often cue.
