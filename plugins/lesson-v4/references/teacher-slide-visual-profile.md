...
The telling-to-asking boundary is always a break. A statement followed by the question or instruction it sets up splits there, and a question takes its blue per Semantic colour - the same boundary drives both the break and the colour, so a child sees at a glance where the explanation stops and their job starts.

Two questions never share a paragraph. A child answers one thing at a time, so each question a slide asks starts on its own line, in its own card where the template gives cards, and a scenario that ends in its question breaks before the question (the teacher asked for this after decks where questions ran on inside one block, 8 September 2026). Numbered questions already do this; the rule is for questions inside ordinary text.
...
- **House blue is the colour of a question to children, and of nothing else on the slide.** When the board asks the class something they answer - `Which fan needed electricity?`, `How did children use these two classrooms?` - those words are blue, and a child scanning a busy board finds their question at once. Blue also organises slide titles, short option labels, category names and other secondary organising labels; the templates apply that themselves and it is not a decision to make per line.
- Black carries everything else the board says: teacher explanation, supporting prose, statements, takeaways, success-criteria body text, reminder body text, and the instructions children act on. `Explain your answer using the photograph.`, `Point to the details that support your comparison.` and `Choose the same part of classroom life in both photographs.` are all black. An instruction that follows a blue question is already read as part of that question's job, and painting it blue too spends the very contrast that was lifting the question - a whole board arriving blue tells a child nothing (flagged by the user, 3 September 2026). Whether a child could act on it now makes no difference here: a task is black either way, because it is a task and not a question.
- **The starter is the one place a question is normally black.** A starter is questions all the way down, so blue there marks nothing a child cannot already see, and a screenful of solid blue is harder to read than a screenful of black. Leave a single starter question black. Where a starter carries several, alternate them black, blue, black, blue: on that slide the colour has a different job, separating one numbered question from the next, exactly as `peer-blue` and `peer-purple` do for peer prompts. The starter answers slide keeps its answer green.
- The treatment is deck-wide: every child-facing question outside the starter carries it, not a favoured few. The carrying fields are `color` or `focus-blue` for a whole line and `[[ ]]` for a span inside a line.
- The boundary is the sentence, not the block. A block that first tells and then asks splits at that boundary: the telling stays black, the asking goes blue, and a paragraph break sits between them so a child sees where the explanation stops and their job starts. `A kettle and a lamp are both appliances.` is black; `What is electricity doing in each one?` follows on its own line in blue. Never colour a whole mixed block blue for the question buried inside it - that hides the question instead of lifting it. **The order does not matter.** A block that ASKS and then instructs splits at the same boundary the other way round: the question goes blue, the instruction that follows it stays black. `Is Isla correct?` is blue and `Explain your answer.` is black beneath it. Only the telling-then-asking order was written down, so an Apply slide holding `Is she correct? Explain your answer.` as one line printed the whole thing black, and the teacher coloured the question by hand (19 September 2026): "even if I put paragraph breaks between them, they're still all black text. So I thought, why not make the question blue?" A block that asks twice is two questions and both are blue; the test is what the sentence DOES, never where it sits.
...
- `task-action` marks an existing action verb or short action phrase such as `Build`, `Test`, `Record`, `Compare` or `Explain` when exposing those verbs makes a multi-phase task easier to scan. It and `core-action` render bold in the line's own colour, so an instruction stays black however many verbs it exposes; a Year 4 PSHE deck once had `Choose`, `Draw`, `Label` and `add arrows` all in question blue, one task sentence in four alternating chunks, and the question on the same slide had nothing left to lift it (8 September 2026). Several `task-action` spans may appear in one instruction. Do not mark every verb in ordinary prose.
- Sticky-knowledge purple is the third member of the grammar, and the builder applies it for you: the `✨` at the head of a sticky line makes that sentence purple, so a child reads at a glance that this is the one to keep. Leave it alone. An `emphasis` span stretched across the whole statement repaints it, which is how a deck ends up with its one sticky fact in red and no purple anywhere; mark a span inside a sticky line only when that span really is a taught term of its own. The check refuses a whole-line emphasis on a sticky line by name.
- **Supplied orange is the information the question hands a child to work from, and on a slide that sets a task from a case, that case is the supplied thing.** Its existing job is a value inside a question (`<<358>>`); the same job at paragraph length is Sam sipping squash all afternoon with a hole in his back tooth. The teacher coloured it that way himself on 18 September 2026 and it is right: the board then has three plain jobs a child reads at a glance - black for what is generally true, orange for what happened to this person, and the line naming what they have to do. Carry it to every later slide that shows the same case, so the words keep one colour wherever a child meets them (Repeated-reference identity, below). It is not for a case the class is only being told about in the teaching; it marks the one the task runs on.
- `problem-state` red is reserved for source wording that names a genuine failed or wrong state.
- `safety-warning` red is reserved for an exact source-authored safety warning. Safety and failure are different semantic roles even though both use the problem-red palette. Neither is a way of making a sentence look important: a sticky-knowledge line is not a warning because it is about safety, it is the thing to remember, and it reads purple like every other one.
- Vocabulary green marks this lesson's taught terms wherever a child reads them: in task text, teaching sentences, sticky knowledge, success criteria and captions alike, every occurrence, so the word looks the same on slide 12 as it did on its vocabulary card and a child recognises it as the word they were taught. Mark each occurrence with the `vocabulary` emphasis role. Success criteria arrive with the taught word already marked as `{{word}}` in the lesson designer's own wording (`preferences.md` → Success Criteria), and that mark is what colours it there. The terms are the lesson's vocabulary cards (and a prior lesson's term the design says children already hold), not every subject noun on the slide: `biome` and `climate` are green on a lesson that taught them; `country` is not. Answer reveals keep their own green, so a taught term inside a revealed answer is not marked twice. Answer green belongs only to answers being revealed or marked - and on an answer slide it belongs on the answer words themselves. A green outline around black words is a frame, not an answer treatment.

**One line of a Teach slide's explanation may be orange, and only one.** A wall of black teaching is hard to take in however well it is written, and the teacher asked for a way to break it that is not only layout (12 September 2026). Orange is free here: its existing text job, `<<supplied-orange>>`, tints a value the question hands a child to work from, which belongs to a slide that asks something, and a Teach slide is not asking. So where the explanation runs to more than one line and one of them carries the weight, give that line `color` orange, which on a teach layout is `"orange": true` on that line. The test is which line you would say louder.

Three bounds, and they matter more than the permission. **One line per slide**, or the colour stops separating anything and the slide is a wall in two shades instead of one. **Not on every Teach slide**: a deck where the same middle line goes orange every time has a tic rather than a voice, and the honest answer on many slides is that layout has already done it, which is what a lesson's own recomposed slides showed when the first line was lifted to full width and needed nothing else. And it **never touches the sticky line**, which is purple because it is the one to keep, nor a question, which is blue, **nor a line carrying a taught vocabulary term**. A PSHE slide put a whole explanation line in orange around the green word `Oxygen` and the term stopped reading as the word children were taught and started reading as one more colour in a scheme (12 September 2026). Vocabulary green marks every occurrence of a taught term wherever a child reads it, so the term wins and the line loses: choose a different line for the orange, and if every line carries a term, leave them all black and let layout do it.

Outside that one case, prominence on a black explanatory line is still size, position and spacing rather than colour.
