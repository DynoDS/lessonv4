...
- **Blue is about now, not about grammar.** An imperative is not automatically an instruction: a rule of conduct phrased as one is still the board telling children something, and it stays black. `Keep real people's private stories private.` and `Always wash your hands before you cook.` are rules the lesson is teaching - they name how to behave every time, not what to do on this slide, and a child cannot act on them at that moment. `Discuss made-up people and events.` is the same sentence twice over: black while the teacher is explaining what a good discussion does, blue on the slide that actually sets the class discussing. The test is whether a child could do the thing now, from this slide, and know when they had finished it. A whole card of blue that nobody can act on is the failure this line exists to stop, and splitting a mixed card into two blue cards does not repair it - it just moves the telling somewhere the check cannot see.
