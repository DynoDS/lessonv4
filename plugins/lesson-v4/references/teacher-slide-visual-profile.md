...
- `task-action` marks an existing action verb or short action phrase such as `Build`, `Test`, `Record`, `Compare` or `Explain` when exposing those verbs makes a multi-phase task easier to scan. Several `task-action` spans may appear in one instruction. Do not colour every verb in ordinary prose, and do not stretch one across the instruction it sits in: that is the whole-line blue the rule above refuses, arriving as a span.
