...
**Test-like means the register of the tests THIS age group sits, and the understanding test applies to worksheet wording at full strength.** A real KS2 paper asks one thing at a time in short direct words: `Give two ways...`, `Explain why...`, `Tick one.` The licence to be neutral and test-like is a licence for that register, not for the compressed wording of an older exam: a Year 4 sheet asking a child to `identify the body job that is less represented` or `justify one targeted improvement` has borrowed a GCSE essay voice, and the child spends their thinking on parsing instead of on the task. Three habits keep worksheet prompts honest. An instruction is a short imperative (`Draw lines to match.`), with no preamble (`For this task, ...`) in front of it. A prompt never lists choices the page already prints - where tick boxes, a word bank or match cards show the options, the prompt names the decision and the printed options do the rest, because the same words in two places is twice the reading for no more thinking. And a name invented during planning (`the balance rule`, `the nutrient-job table`) never reaches the child: the sheet says `the table above`, because the child has met the thing, not its working title.
