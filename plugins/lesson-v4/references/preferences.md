...
**Plain means direct, not exam-flavoured - a worksheet is not a test.** What a worksheet drops is the slide's flavour, never the child-level language: it asks one thing at a time in the short direct words this age group can act on (`Give two ways...`, `Explain why...`, `Tick one.`), and the understanding test above applies to its wording at full strength. A Year 4 sheet asking a child to `identify the body job that is less represented` or `justify one targeted improvement` has slid from plain into an exam-paper voice, and the child spends their thinking on parsing instead of on the task. Three habits keep worksheet prompts plain. An instruction is a short imperative (`Draw lines to match.`), with no preamble (`For this task, ...`) in front of it. A prompt never lists choices the page already prints - where tick boxes, a word bank or match cards show the options, the prompt names the decision and the printed options do the rest, because the same words in two places is twice the reading for no more thinking. And a name invented during planning (`the balance rule`, `the nutrient-job table`) never reaches the child: the sheet says `the table above`, because the child has met the thing, not its working title.
