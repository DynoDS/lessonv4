...
**Question (1) starts at the top left, and the run flows so the next number is easy to find.** The anchor is fixed: a child picks the sheet up and finds (1) where a page begins, top left - never top right while a stimulus takes the left (Daniel, 29 August 2026). From there the flow is a judgement, not a pattern to enforce: usually that means the run continues down a column, because (2) directly under (1) is the easiest find there is, but a long (1) that fills one side with (2) beside it reads perfectly well. The test is whether a child who has just finished a question can see where the next one is without hunting; a run that ping-pongs left, right, left, right between short zones fails it, and the mirrored and turned layout variants exist so a followable arrangement is always available. When a sheet pairs its question run with one shared evidence panel (a map with photographs, a source set, a data table every question uses), this teacher wants the questions on the left and the panel on the right beside them: the child keeps their place in the run and glances right at the evidence. This organises the page; it does not change what a child must have met before they can act. Material a question directly quotes or completes - a claim being judged, a stem being finished - still reaches the child before the question that leans on it.
