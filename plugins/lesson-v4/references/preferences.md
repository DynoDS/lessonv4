...
**Questions read down a column, and a shared evidence panel sits to their right.** A child works a sheet the way they read a book page: downwards. When the question run splits across side-by-side zones, choose an arrangement where the flow runs down one column and then down the next, so (2) sits under (1) rather than across the page from it - a sheet whose questions zigzag left, right, left, right makes the child hunt for the next number, and the mirrored and turned layout variants exist precisely so the down-a-column arrangement is always available. When a sheet pairs its question run with one shared evidence panel (a map with photographs, a source set, a data table every question uses), this teacher wants the questions in the left column and the panel on the right beside them: the child keeps their place in the run and glances right at the evidence (Daniel, 29 August 2026). This organises the page; it does not change what a child must have met before they can act. Material a question directly quotes or completes - a claim being judged, a stem being finished - still reaches the child before the question that leans on it.
