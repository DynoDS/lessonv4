...
**A lesson reads as one line.** A teacher flicking through the deck should be able to say where it is going: each beat is one step that hands on to the next, from the opening to the final performance. Two tells that the line has broken. A beat carrying a second job that has no beat of its own (a routine appended to the conclusion, a fact bolted onto a task): children finish one thing and meet a new thing with nothing to do with it. And a run of slides that are all the teacher talking: the rhythm above is what keeps the line moving, because each thing children do is where the lesson turns. This asks for the beats to be in an order that hands on, not for a narrative device or a story-shaped hook.
...
**Black tells, blue asks.** Keep ordinary teacher explanation, statements, takeaways, success-criteria body text and reminder body text black. Use house blue for the words that ask children something or tell them to do something - the questions they answer and the instructions they act on - and for slide titles, short category names, option labels and other small navigational labels. A block that first tells and then asks splits at that boundary: the telling stays black, the asking goes blue on its own line. The full grammar lives in `teacher-slide-visual-profile.md` → Semantic colour, which owns this rule.
