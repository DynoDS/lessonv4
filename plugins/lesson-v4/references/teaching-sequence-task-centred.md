...
```json
{
  "kind": "teach-needed",
  "content": {
    "enablingInput": "the one idea children need, as the child reads it: what it means, why it matters and what it looks like, in short lines, never the rule alone",
...
```json
{
  "kind": "do-task",
  "content": {
    "activity": "the sustained independent work and the artefact it produces, opened by the line naming what the lesson has established and the steps children take",
    "planWithinTask": null,
...
Attach required visuals through `representationRefs` and `photoRefs`; attach the exact standard through `successCriteriaRefs`. Put answers/models only in `answer` and teacher wording only in `speakerNotes`.
