...
```json
{
  ...
  "content": {
    ...
    "explanation": "only what the thing on the board and the landed sentence cannot say for themselves, in two or three short lines; null when they can",
    "takeaway": null,
    "teachingText": null,
    "keyQuestions": [
...
`explanation` is what the board has to say that the thing on it and the landed sentence cannot: the cause behind what the class can see, a number with its unit, the boundary of a definition, the meaning of a source. Two or three short lines the slide designer keeps as lines. Use `null` when the example, the question and the one landed sentence already carry the teaching and the teacher's voice does the rest, which is the shape of the pride lessons: a name, a convention, a fact that simply is so beside its example, an object that shows its own meaning. Use lines for a rule that cannot show its reason, a source whose meaning is not on its face, a cause behind what the class can see; the failure at this end was a PSHE slide that put `You can pass without giving a reason` on the board and left what pass means in the script. What it is not is the landed sentence again in other words: a teeth slide carried `Incisors cut; canines help tear.` as its headline, a card saying `Their thin biting edges meet to cut through food` and a star line `Incisors cut food and canines help tear food.`, one sentence three ways. The validator refuses the near-copy it can measure (the headline and the star line); the card that restates the headline with one detail added is yours to refuse. Where the example is a rule and cannot show its own reason, the explanation earns its lines. For the right to pass: `Sometimes you don't want to answer a question or share something personal. In PSHE, you can choose to pass.` / `You can say "I'd like to pass." You don't have to explain why.`
