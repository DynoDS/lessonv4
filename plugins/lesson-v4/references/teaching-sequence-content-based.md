...
A chunk is knowledge: what happened, what somewhere was like, how something works, who wanted what. A thinking move the subject uses (a deduction from a source, a comparison across time, the limit of one piece of evidence) is taught in one or two lines inside the beat where the material makes children need it, never as a chunk of its own, because a rule about method taught before the thing it is a method for is a slide children cannot see the point of. Read the finished Teach headlines in order: they should be things about the topic, and a run of rules about how to think is the method standing where the knowledge should be. The subject file says how the move sits inside the beat in its subject.

**So the lesson opens on the thing, and the first question is one children can answer by looking at it.** Put the photograph, the map, the object, the result or the account on the board, big, and ask what they notice, what is like theirs, what is different. Children are then doing the lesson inside the first two minutes, on real material, which is the subject's own move before it has a name. Teach the idea next, give it its name, and attach the name straight back to what they just noticed, so the word lands on something they have seen rather than arriving as a definition to hold. The subject's caution or rule comes later still, at the point where the knowledge makes children need it, as a question they can now answer.

The trap this avoids has one shape and many costumes, so name your subject's version of it before you start:

- **History** opening on how to read a source instead of on the source, so the class meets rules about evidence before it meets anybody who lived.
- **Geography** opening on how to use a map or a grid reference instead of on the place, so children learn a procedure before they know where or what they are looking at.
- **Science** opening on the definition or the labelled diagram instead of on the thing happening, so the word arrives before the phenomenon that makes it mean anything.
- **Religious education** opening on what a concept means instead of on the practice, story or object it lives in, so belief becomes vocabulary.
- **Personal, social and health education** opening on the rule instead of on the situation, so children are told the answer to a question nobody has felt yet.

The limit: a lesson whose material genuinely cannot be met first still teaches straight. A fact with no visible referent, a convention, a name, or knowledge children need before the material makes sense is taught plainly at the front, and the subject file's own routing wins wherever it is more specific than this.
