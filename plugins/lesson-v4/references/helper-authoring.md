...
**Record the answer where the guard can hold you to it.** Every entry in `shared/visual-parity.js` carries a `depicts` field, and it has exactly two values: `'data'` when the drawing is right by matching the lesson's own data or an agreed convention, and `'asset:<folder under builder/assets>'` when it depicts a real place or object and takes its form from a file this package ships. `npm run check` refuses an entry that declares neither, and refuses an `asset:` folder that does not exist. The guard cannot judge whether a drawing is accurate; what it can do is make you write down what it is drawn from, so "I picked these coordinates by eye" has to be said out loud before it can ship.

**A projection is not an answer, and used to be accepted as one.** The guard once took `'projection:<named projection>'` as a third value, meaning "the geometry comes from real longitude and latitude". A schematic world map declared `projection:equirectangular-lonlat` - a genuinely real projection - over continent outlines somebody had typed out to look about right, and passed. It went on to draw the opening eight slides of a Year 4 lesson about where the Amazon actually is. A projection says how coordinates are **transformed**; it says nothing about where they **came from**, so it can never be evidence, and no list of allowed projection names could have caught this one because the name was already correct. Real coordinates still reach the page - the globe-to-flat presentation reprojects the shipped world into a globe - but what it reprojects is the asset, so it declares `asset:maps` like everything else. If you find yourself wanting to say "these coordinates are real", the way to prove it is to put them in a file under `builder/assets/`.
...
- **Where it draws: everywhere.** Wire the picture into all four surfaces and list it in all four designers' catalogues. Whether a particular lesson puts it on the wall or in the stick-in pack is the designers' teaching decision at the time, and they can only make it if the surface can draw it. Availability is not pedagogy; use is.
- **How it draws: from the shared module only.** A surface passes the box it has and its profile (`shared/visuals/surface-profiles.js`) and places what comes back. The profile carries what genuinely differs by surface: how big words must print to be read there, whether the board may grow the drawing into spare room, and that the stick-in pack prints in ink because it is photocopied. Shapes, proportions, marks, colours and the spec's field names are the same on all four. A surface that wants something different is asking for a change to the shared drawing, which then reaches every surface.

**Lay it out at the size it will print.** Take `widthPt` (and, on the board, `heightPt`) from the profile and set every font in points, so a readable floor means the same on a slide as on a sheet and a drawing squeezed into a shallow zone gives up rules and gaps before its words, then refuses by name. `number-line-svg.js` is the worked reference: `tightSvg(spec, profile)` returns `{ svg, w, h, aspect, layout }` in points, and `cacheKey(spec, profile)` includes the box.

**Read the spellings the surfaces already use.** When you move an existing picture into its shared module, each surface's designers have been writing its fields in their own words (the wall's number line said `from`, `to`, `step`; the stick-in piece counted `intervals`). Read those in the shared module so every spec already written still draws, and document one spelling, the board's, going forward.

**Typed layout is not a picture.** Questions, writing lines, word banks, frames of labelled lines, tables of words and success-criteria panels are set as each surface's own text, so a teacher can edit the words in PowerPoint and a sheet can wrap them to its column. They are listed in `TYPED_LAYOUT` and `WORKSHEET_LAYOUT_EXEMPT` in `shared/visual-parity.js`. The test is what carries the meaning: marks whose position or shape means something are a picture; words in an arrangement are layout.

**The guard holds all of this, and it reads the code rather than a declaration.** `builder/scripts/sharing-status.js` works out, for every picture and surface, whether that surface draws it from its `geometrySource` module (`shared`), with code of its own (`own`), or not at all (`missing`), and `npm run check` refuses any difference from `SHARING_BACKLOG` in `shared/visual-parity.js`. The backlog lists what was not shared on 13 September 2026 and may only shrink: a surface you move onto the shared drawing comes off it in the same change, and nothing new may be added, so a new helper that is not shared on all four surfaces does not pass. Every worksheet helper must be a picture in the manifest or listed as typed layout, so a new drawing on paper cannot slip past either.
...
1. **The placer, not a slide file.** Add the picture to `FIGURES` in `builder/src/content/shared-figure.js` (`type` -> its shared module and a plain name). That one file places every shared drawing on a slide: it lays the drawing out at the zone's real size during the preflight, the pictures are made between the preflight and the real pass, and the real pass places them. A drawing that refuses its zone refuses in the preflight, by name.
2. **The dispatcher** — `builder/src/content/index.js`, in two places: the `HELPERS` map (`type` → `drawerFor('<type>')`) and `ZONE_COMPAT` (`type` → which zone classes it fits). A type missing from either falls back to plain label text.
3. **The catalogue**: `references/templates.md` §4 (content objects): a one-line purpose in the §1.2 table, then a `### <type>` section with a JSON example and the field list. This is the only thing the slide-designer reads to learn the helper exists; an unlisted helper is never emitted. The first JSON example whose `type` matches is built on a full slide by `builder/test/guide-examples-build.test.js` on every check, so write one that is real, vocabulary-free of placeholders and buildable: an example that cannot be drawn is a helper nobody can use.
4. **Nothing to pre-render.** The older shared figures each carry a `preRender<Name>` in `builder/build.js` that renders at one size before any zone is known; a picture placed through `shared-figure.js` does not need one.
...
6. **Vocabulary cards: nothing to wire.** The key-vocabulary card draws any content type the dispatcher knows and sizes its panel from the picture's own ink, so a new helper works on a vocabulary card the day it is registered. Only if the helper is text-and-parts content read across its width (a table, a chart, a set of cards) add its type to `STACKED_VISUALS` in `builder/src/templates/key-vocabulary.js`, so it goes under the word at full width instead of beside it; render it on a vocabulary card to check.
...
A wall figure renders from the same `shared/visuals/<name>-svg.js` module the slides use, so once that shared module exists the wiring is short. Every picture is wired here; whether a wall uses it is the wall designer's call.

1. **The SVG registry** — `working-wall-html/src/svg-renderer.js`: `require('../../shared/visuals/<name>-svg')` and add it to the `PRIMITIVES` map in the aspect-true form, `<type>: { keyFn: <key with the wall profile>, tightFn: (s) => shared.tightSvg(s, <the wall profile>), collected: {} }`, mirroring `numberLine`. Export its `<name>Key` from the module too.
...
A stick-in piece renders from the same `shared/visuals/<name>-svg.js` module, so the wiring is short once that exists. Every picture is wired here. The stick-in-sheets designer uses a piece for a write-on moment (a picture the child marks, sorts, labels or draws on) or a read-from moment, and that choice is its own; the pack only has to be able to print whatever it chooses.

1. **The visual registry** — `stick-in-sheets-html/src/visual-registry.js`: add `<type>: { geometry: <name>, laidOutAtWidth: true, tightSvg: (spec, box) => <name>.tightSvg(spec, profileFor("stickin", box)), defaultWidthMm: … }` to the `VISUALS` map, mirroring `number-line` (or `ROW_VISUALS` for a `<type>-row` strip of figures, each with its own write-on line). A figure that needs bespoke handling (a base directory for a photo, a reserved label band) dispatches through an `item.visual === "<type>"` branch in `stick-in-sheets-html/src/render-piece-html.js`'s `renderPieceHtml` instead, mirroring `label-diagram`. Size by the smallest comfortably-usable size so the most copies fit a page.
