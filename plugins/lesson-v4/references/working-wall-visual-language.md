...
An exact row/column lookup table carried over from the final slides also qualifies as a P1 visual reference when its relationships are the support children recognise and use. Preserve its headers and rows. It does not need an unrelated photograph alongside it; arbitrary prose placed in cells does not qualify. This is the same-form reference-table support required by the designer's visual-matching rule.
