...
P3 decoration is outside this entry ticket. It may coexist with an already
wall-worthy ordinary card, but it never makes a card wall-worthy and never
substitutes for the lesson diagram, meaningful photograph or genuine P2 cue.
P3 is supported only on `stickyKnowledge`, `workedExample`, `sentenceStem`,
`misconception`, `referenceTable`, `equivalenceGrid`.

The wall is read picture-first (that is the whole point of design move 3 below): a child glances from four metres away and takes in the picture before a single word. So a visual is what earns a card its place on the wall, not an upgrade added once the words are written. A card carries a visual a child recognises by sight, and the words become its caption. When a card has no honest visual, it belongs on the slides and the worksheet, where children read close up at a desk; on the wall it would only be a block of text nobody can use from across the room.

A visual is any of these, in order of preference:

- **The lesson's own drawn diagram** - the primitive the slides used (a clock, a fraction bar, an angle, a Venn, a bar model). The wall shows the same shape children saw on the board, so the support carries from desk to wall. This is the strongest visual and the first choice whenever the lesson has one.
- **A photograph the lesson already fetched** - a filename already in `photo-requirements.json`, used only where it genuinely anchors the idea (pizza slices for fractions, a coastline for a geography feature). Reuse what the lesson has rather than going out of your way to source new images.
- **A genuine P2 context picture** - where no lesson diagram or required photo fits, a small card-level cue that makes the nearby meaning recognisable: either a complete emoji `picture` or a resolved Educational SVG `picture`. Keep protected lesson wording unchanged; the picture sits in the card's visual area rather than being typed into an upstream string. A child who cannot yet read the words can still recognise the cue.

The one card that may stand on words alone is a **step-by-step success-criteria card**, and only on a lesson that has no picture to give it: nothing the picture stage published, nothing its slides drew. The ordered steps are themselves the thing children look up, so the procedure is its own support when there is nothing to show it with. Where a picture does exist this card takes it like any other, and the wall's deterministic check refuses a wordless card while one is available. Even here, show the action wherever a primitive can draw it: pair the steps with the lesson's anchor diagram, or a picture of the step happening (a number with the tens column highlighted beside *Find the tens column*), so the card shows as well as tells. When the action needs a picture no primitive draws yet, note it in your final report so it gets built, rather than letting bare steps quietly become the normal.

Choosing the visual is judgement, not box-ticking. A visual earns its place by making the meaning land faster from across the room; a stock photo bolted onto an abstract card to satisfy the rule adds clutter, not understanding. When no P1 visual fits, a genuine P2 cue may be the card's entry ticket. If an Educational SVG lookup fails, use its suitable emoji fallback when one exists; otherwise a card that has no other qualifying visual stays on the slides, except for the existing step-by-step success-criteria exception.
...
**Your job:** carry a visual on every card (the entry-ticket principle above), and reach first for the lesson's own drawn shape when it has one - a clock face, a fraction circle, an angle, a pair of lines, a fraction bar, a number line. The diagram does the heavy lifting; the text becomes the supporting caption. Where the lesson has no drawable diagram, the visual is a photo it already fetched or a genuine P2 context picture. The full set of decision rules is in the "When to attach a `visual`" section below.
...
3. **The card is a worked-example, sticky-knowledge, or future visual-anchored card.** Worked examples and sticky knowledge are where drawn diagrams land most often. Sentence stems and misconceptions rarely need a *drawn diagram* - they're language-driven - but that does not excuse them from carrying an honest qualifying visual. Only a family with a supported card-level picture area (`stickyKnowledge`, `workedExample`, `misconception`) may use a P2 `picture`; a `sentenceStem` cannot, so its visual must be a legitimate P1 diagram or an already-fetched lesson photo. Where no qualifying P1 or P2 visual is available, the ordinary card is omitted rather than shipped as words, the step-by-step success-criteria card being the one exception.
...
- **The card is the step-by-step success-criteria exception.** A full procedure card (six numbered steps) may stand on its steps - cramming a clock onto the side would shrink the steps below floor size. Show the action with the lesson's anchor diagram where a primitive can draw it; otherwise let the numbered method be the support and note any missing action-picture in your final report.

When none of these gives an honest visual and the card is not a success-criteria card, it stays on the slides rather than going up as text. Note that choice in `rationaleNote`, and note any primitive you wished for in your final report - that's how the next renderer build knows what to add next.
