...
The one card that may stand on words alone is a **step-by-step success-criteria card** - the numbered method children come back to in the next lesson. The ordered steps are themselves the thing children look up, so the procedure is its own support. Even here, show the action wherever a primitive can draw it: pair the steps with the lesson's anchor diagram, or a picture of the step happening (a number with the tens column highlighted beside *Find the tens column*), so the card shows as well as tells. When the action needs a picture no primitive draws yet, note it in your final report so it gets built, rather than letting bare steps quietly become the normal.
