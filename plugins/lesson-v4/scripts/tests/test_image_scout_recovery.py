...
class LedgerSafetyTests(unittest.TestCase):
    ...
    def test_no_output_is_not_a_rejection_and_keeps_the_recovery_call(self):
        """A call that produced nothing has not been judged; it has failed.

        Year 4 Science, 22 September 2026. ImageGen returned
        `imagegen_output_unavailable` for the lesson's only anatomical diagram.
        The scout completed the attempt as `rejected`, which is the one outcome
        that authorises no further call, and the ledger accepted a rejection of
        an image that was never produced (`"staging_path": null`). A replacement
        was generated by hand and checked, and there was no legal way to adopt
        it. Every outcome in `image-scout-generation.md` classifies an output;
        none of them applies when there is no output.
        """
        self.reserve()
        with self.assertRaises(attempts.LedgerError) as caught:
            self.complete("rejected", fault="imagegen_output_unavailable")
        self.assertIn("interrupt-open", str(caught.exception))

        # The honest route keeps the second call the run needed.
        attempts.cmd_interrupt_open(self.args(reason="imagegen_output_unavailable"))
        self.assertEqual(
            attempts.cmd_status(self.args())["attempts"][0]["state"], "interrupted"
        )
        self.reserve("recovery prompt", "recovery")
        self.complete("accepted", attempt=2, staging_path=str(self.make_image()))
        self.assertEqual(attempts.cmd_status(self.args())["attempts_used"], 2)

    def test_a_rejection_of_a_real_image_is_still_a_rejection(self):
        """The boundary: the gate is about the missing image, not the verdict.

        A scout that looked at a generated picture and judged it a fundamental
        miss is making exactly the call this outcome exists for, and it still
        closes the door.
        """
        self.reserve()
        attempts.cmd_record_generated(self.args(staging_path=str(self.make_image())))
        self.complete("rejected", fault="wrong concept entirely")
        self.assertEqual(
            attempts.cmd_status(self.args())["attempts"][0]["outcome"], "rejected"
        )

    def test_fundamental_miss_does_not_spend_attempt_two(self):
        self.reserve()
        attempts.cmd_record_generated(self.args(staging_path=str(self.make_image())))
        self.complete("rejected", fault="fundamental miss")
        before = self.ledger_path().read_bytes()
        with self.assertRaises(attempts.LedgerError):
