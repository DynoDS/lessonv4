...
def valid_content_contract():
    ...
    set_route(
        ...
        [
            ...
            source_unit(2, "teach", {"headline": "Roads open up the forest", "explanation": "Think about how you get to school: there is a road, so anyone can come. Before this road was built, the only way into this part of the forest was on foot or by river. Look at the photograph: the road runs straight through, and lorries can now drive to trees nobody could reach before.", "takeaway": {"kind": "text", "text": "A road can let more people reach the forest."}, "teachingText": None, "keyQuestions": ["What might happen once a road is there?"]}),
            ...
            source_unit(4, "practise", {"launch": {"established": None, "goodLooksLike": {"strong": {"words": "The river path was the only way in, so few people came. A road lets lorries reach the trees, so more land is cleared.", "show": None}, "weak": {"words": "Roads are bad for the forest.", "show": None}, "difference": "The strong one says how each step leads to the next."}, "steps": []}, "reasoningWords": ["because", "so", "as a result"], "rehearsal": {"sayIt": "Tell your partner how the road leads to more forest being cleared.", "partnerAsks": "Why does that matter?"}, "activity": "Explain the chain", "format": "short written explanation", "task": "Explain how a new road could lead to more forest being cleared."}, answer={"kind": "model", "content": "The road makes the area easier to reach, so more people may enter and clear land.", "acceptanceCondition": "Accept an accurate causal explanation.", "delivery": "answer-slide"}),
        ],
    )
...
def valid_task_contract():
    ...
    set_route(
        ...
        [
            ...
            source_unit(3, "do-task", {"launch": None, "reasoningWords": None, "rehearsal": None, "activity": "Plan the comparison, get the fairness check, then run the investigation.", "planWithinTask": "Choose what will stay the same while the material changes.", "checkpointQuestion": "What would make this unfair?", "runsBeyondToday": False, "todayEndsAt": None}),
