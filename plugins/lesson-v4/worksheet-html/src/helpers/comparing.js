...
// nested helpers, so this helper owns only the child's < > = target. It is the
// one comparison ring (shared/visuals/comparison-svg.js) the board's
// comparison-slot and the wall's comparison card draw, so a child writes into
// the same ring on paper as on the board; it was a blue rounded square of the
// sheet's own until 13 September 2026.
...
const helpers = {
  "comparison-target": atPrintedWidth(comparisonShared, { minWidthMm: COMPARE_BOX_MM }),
  "compare-row": {
