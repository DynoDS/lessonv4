...
const { TYPE, RULE, INSET } = require("../tokens");
const {
  BODY_PT,
  PT_MM,
  LINE_MM,
...
function bubbleSize(bubble) {
  ...
  const labelW = bubble.label
    ? String(bubble.label).length * (PW.labelFont * 0.55)
    : 0;
  const innerW = Math.max(coinW, labelW, PW.minInnerW);
  const innerH =
    coinH + (bubble.label ? PW.labelFont + (coins.length > 0 ? PW.labelGap : 0) : 0);
...
function buildPartWholeSvg(spec) {
  ...
  const partsRowW =
    partSizes.reduce((sum, s) => sum + s.w, 0) + (parts.length - 1) * PW.partGap;
  ...
  function drawBubble(bubble, x, y, size) {
    ...
    if (bubble.label) {
      // Ink, not the "given" orange, and deliberately: the orange belongs to
      // the coins, which are the material the lesson hands over. A bubble's
      // label is as often a prompt the child reads ("Left", "Pounds") as a
      // value they are given, and one word cannot be two colours.
      els.push(
        `<text x="${f(x + size.w / 2)}" y="${f(cy + PW.labelFont * 0.85)}" text-anchor="middle"` +
          ` font-family="var(--font)" font-size="${PW.labelFont}" fill="var(--colour-ink)">${esc(bubble.label)}</text>`
      );
    }
  }
  ...
  const narrowestBubble = Math.min(
    wholeSize.w,
    ...partSizes.map((s) => s.w)
  );
...
function partWholeMinWidthMm(geometry) {
  const byBubble = (PW_BUBBLE_MIN_MM / geometry.narrowestBubble) * geometry.unitsWide;
...
function partWholeMaxWidthMm(geometry) {
  const byBubble = (PW_BUBBLE_MAX_MM / geometry.narrowestBubble) * geometry.unitsWide;
  const byCoin = geometry.hasCoins
...
function renderPartWholeMoney(spec) {
  const geometry = buildPartWholeSvg(spec);
  const stem = spec.text ? `<p class="h-money-stem">${esc(spec.text)}</p>` : "";
  return `
    <div class="h-money">
      ${stem}
      <div class="h-money-figure" style="max-width:${f(partWholeMaxWidthMm(geometry))}mm">${geometry.svg}</div>
    </div>`;
}

function measurePartWholeMoney(spec, widthMm) {
  const geometry = buildPartWholeSvg(spec);
  const stemMm = spec.text
    ? linesFor(spec.text, widthMm) * LINE_MM + SPACE_TIGHT_MM
    : 0;
  const drawnWidth = Math.min(widthMm, partWholeMaxWidthMm(geometry));
  return stemMm + drawnWidth / geometry.aspect;
}

function needsPartWholeMoney(spec) {
  const geometry = buildPartWholeSvg(spec);
  ...
  return {
    minWidthMm,
    minHeightMm: minWidthMm / geometry.aspect + stemMm,
  };
}
...
const helpers = {
  ...
  "part-whole-money": {
    render: renderPartWholeMoney,
    measure: measurePartWholeMoney,
    needs: needsPartWholeMoney,
    greed: NEVER_STRETCH,
  },
  "chip-bank": {
