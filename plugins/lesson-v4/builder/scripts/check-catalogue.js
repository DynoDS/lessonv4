...
//   wall   — builder VISUAL_KEY_FNS  vs  agents/working-wall-designer.md (its
//            supported-primitives list, the file that gates what the wall-designer
//            emits). This is the systematic version of the by-hand reconciliation
...
{
  const md = fs.readFileSync(ref('agents', 'working-wall-designer.md'), 'utf8');
  const keys = Object.keys(VISUAL_KEY_FNS);
  const missing = keys.filter((k) => !documentedIn(md, k));
  if (missing.length) {
    problems.push(
      'Wall catalogue gap — these built wall primitives are not in agents/working-wall-designer.md, so the wall-designer believes it cannot draw them and ships words instead:\n' +
      missing.map((k) => '    - ' + k).join('\n') +
      '\n  Fix: add each to the designer\'s supported-primitives list and its "Visual primitives" table.'
    );
  } else {
    console.log(`Wall catalogue OK: every built wall primitive (${keys.length}) is documented in working-wall-designer.md.`);
  }
}
