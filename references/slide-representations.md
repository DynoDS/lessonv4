...
**Question and reference** shows the exact question, success criteria and every useful representation or dataset while written working happens away from the helper. `writing-turn-ref-sc` is often a suitable shape because it has no generic working column, but template choice follows the actual question and references rather than a named classroom surface. Put a fixed dataset in `reference`, the SC in `criteria`, and leave the reference empty only when there genuinely is none.
...
## The generic working-space flag stays off

`maths-turn-sc` and `maths-turn-ref-sc` both used to add a blank annotation column beside the question by default. That default remains retired: **turn the generic flag off** - `"workingSpace": false` on `maths-turn-sc`, `"hideWorkingSpace": true` on `maths-turn-ref-sc`. With a `questionVisual`, the flag gives the helper the full width it would otherwise share with an unused column; on `maths-turn-ref-sc` with no `questionVisual`, the reference panel expands into the freed space. A Question-and-reference state with no helper normally uses `writing-turn-ref-sc` or another suitable no-working-column template. Judge each genuinely different modelled move by its own resource state rather than assigning one presentation treatment reflexively to the whole lesson.
