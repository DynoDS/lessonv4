...
## Clean-checkout publication and dependency findings

GitHub Actions run `34531535430` reconstructed the exact tested 17-file candidate from the pinned work revision, verified every changed file's SHA-256, passed 205 protected Python tests and 283 subtests, passed the complete builder check command including all 588 builder tests, and passed all five worksheet controls. Only then did it push without force and read back `work` at `0cd35e06a3fa9f5ff0ece00557dcf3876f5b8d7d`. The scratch workflow and patch transport were not included in the work commit. This later evaluation addendum changes no runtime source.

Two preceding scratch publication attempts failed before any push: the first supplied a relative path to the absolute-path plugin-root verifier; the second omitted the Python rendering dependencies from the clean runner. The temporary workflow was corrected to use an absolute path, install the same Pillow/python-pptx/fontTools/lxml versions used locally, and retain command failures through `pipefail`. Production tests were not suppressed and production source was not changed to accommodate those setup errors.

The clean install also ran `npm audit --json`. It reported three high-severity package entries: `image-size` 1.2.1, its dependent `pptxgenjs` 3.12.0, and `sharp` 0.33.5. These are package entries, not three independent vulnerabilities. The report links image-parser advisories `GHSA-w3rx-r6r6-pgpr`, `GHSA-5p2g-fcmc-qvqq`, `GHSA-f88m-g3jw-g9cj` and `GHSA-rgj7-g3m4-5g8c`. The package manifest and lockfile are unchanged from the baseline. No exploitability assessment or dependency remediation was performed; the findings remain open. In particular, the automatic fix suggestion includes a major-version change/downgrade, so no `npm audit fix --force` was applied. Passing the tests does not clear these dependency findings. Dependency remediation and its regression check are an additional pre-release concern alongside the unevaluated live lesson-quality condition.
